...
local colorTemplate = {
	...
	backdropFill = 0,
	borderColor = 0
} -- colors in default fonts and textures
...
MOD.BarGroupTemplate = {
	-- default bar group settings
	enabled = true,
	locked = true,
	merged = false,
	linkSettings = false,
...
MOD.BarGroupLayoutTemplate = {
	-- all the bar group settings involved in layout configuration
	barWidth = 0,
	barHeight = 0,
	iconSize = 0,
	scale = 0,
...
local function Anchor_OnEnter(anchor, bgName)
	...
	if attachment then
		...
		GameTooltip:AddLine(L["Anchor left click 1"], 1, 1, 1)
	else
		GameTooltip:AddLine(L["Anchor left click 2"], 1, 1, 1)
	end
	GameTooltip:AddLine(L["Anchor right click"], 1, 1, 1)
	GameTooltip:AddLine(L["Anchor shift left click"], 1, 1, 1)
	GameTooltip:AddLine(L["Anchor shift right click"], 1, 1, 1)
	GameTooltip:AddLine(L["Anchor alt left click"], 1, 1, 1)
	GameTooltip:AddLine(L["Anchor alt right click"], 1, 1, 1)
	GameTooltip:Show()
end
...
local function GetColorForBar(bg, bar, btype)
	...
	if not c then -- get the best default color for this bar type
		...
		if bt == "Cooldown" then
			c = cc and bg.cooldownColor or MOD.db.global.DefaultCooldownColor
		end
		if bt == "Notification" then
			c = cc and bg.notificationColor or MOD.db.global.DefaultNotificationColor
		end
		if bt == "Broker" then
			c = cc and bg.brokerColor or MOD.db.global.DefaultBrokerColor
		end
		if bt == "Value" then
			c = cc and bg.valueColor or MOD.db.global.DefaultValueColor
		end
	end
...
function MOD:InitializeBarGroup(bp, offsetX, offsetY)
	...
	if not bg then
		bg = MOD.Nest_CreateBarGroup(bp.name)
	end
	if bp.linkSettings then
		UpdateLinkedSettings(bp, false)
	end
	if bp.linkBars then
		UpdateLinkedBars(bp, false)
	end
	if bp.sor == "C" then
		bp.sor = "A"
	end -- fix out-dated sort setting
	if bp.auto then -- initialize the auto bar group filter lists
		if (bp.filterBuff or bp.showBuff) and bp.filterBuffLink then
			UpdateLinkedFilter(bp, false, "Buff")
		end -- shared settings for buffs
		if (bp.filterDebuff or bp.showDebuff) and bp.filterDebuffLink then
			UpdateLinkedFilter(bp, false, "Debuff")
		end -- shared settings for debuffs
		if (bp.filterCooldown or bp.showCooldown) and bp.filterCooldownLink then
			UpdateLinkedFilter(bp, false, "Cooldown")
		end -- shared settings for cooldowns
	end
	if not bp.pointX or not bp.pointY then
		bp.pointX = 0.5 + (offsetX / 600)
		bp.pointXR = nil
		bp.pointY = 0.5 + (offsetY / 600)
		bp.pointYT = nil
	end
	if not bp.pointW or not bp.pointH then
		bp.pointW = MOD.db.global.Defaults.barWidth
		bp.pointH = MOD.db.global.Defaults.barHeight
	end
...
function MOD:LoadBarGroupSettings(bp)
	UpdateLinkedSettings(bp, false)
end

-- Save bar group settings into the linked settings.
function MOD:SaveBarGroupSettings(bp)
	UpdateLinkedSettings(bp, true)
end

-- Load bar group settings from the linked settings.
function MOD:LoadCustomBars(bp)
	UpdateLinkedBars(bp, false)
end

-- Save bar group settings into the linked settings.
function MOD:SaveCustomBars(bp)
	UpdateLinkedBars(bp, true)
end
...
function MOD:SetBarGroupPosition(bp)
	...
	local bg = MOD.Nest_GetBarGroup(bp.name)
	if bg then
		MOD.Nest_SetAnchorPoint(bg, bp.pointX, bp.pointXR, bp.pointY, bp.pointYT, scale, bp.pointW, bp.pointH)
	end
	if bp.linkSettings then
		local settings = MOD.db.global.Settings[bp.name] -- when updating a bar group with linked settings always overwrite position
		if settings then
...
local function SetCache(bg, block, name, value)
	if not bg.cache then
		bg.cache = {}
	end
	if not bg.cache.block then
		bg.cache.block = {}
	end
...
local function GetCache(bg, block, name)
	if not bg.cache or not bg.cache.block then
		return nil
	end
...
local function ResetCache(bg, block)
	if bg.cache and bg.cache.block then
		table.wipe(bg.cache.block)
	end
end
...
function MOD:UpdateBarGroup(bp)
	if bp.enabled then
		if bp.linkSettings then
			UpdateLinkedSettings(bp, true)
		end -- update shared settings in a linked bar group
		if bp.linkBars then
			UpdateLinkedBars(bp, true)
		end -- update shared settings in a linked bar group
		if bp.auto then -- update auto bar group filter lists
			if (bp.filterBuff or bp.showBuff) and bp.filterBuffLink then
				UpdateLinkedFilter(bp, true, "Buff")
			end -- shared settings for buffs
			if (bp.filterDebuff or bp.showDebuff) and bp.filterDebuffLink then
				UpdateLinkedFilter(bp, true, "Debuff")
			end -- shared settings for debuffs
			if (bp.filterCooldown or bp.showCooldown) and bp.filterCooldownLink then
				UpdateLinkedFilter(bp, true, "Cooldown")
			end -- shared settings for buffs
		end
		ResetCache(bp, "Buff")
		ResetCache(bp, "Debuff")
		ResetCache(bp, "Cooldown")
		...
		local bg = MOD.Nest_GetBarGroup(bp.name)
		if not bg then
			MOD:InitializeBarGroup(bp)
			bg = MOD.Nest_GetBarGroup(bp.name)
		end

		if bp.useDefaultTimeFormat then
			MOD:CopyTimeFormat(MOD.db.global.Defaults, bp)
		end
		if bp.useDefaultDimensions then
			MOD:CopyDimensions(MOD.db.global.Defaults, bp)
		end
		if bp.useDefaultFontsAndTextures then
			MOD:CopyFontsAndTextures(MOD.db.global.Defaults, bp)
		end
		...
		if bp.bgtexture then
			bgtexture = media:Fetch("statusbar", bp.bgtexture)
		end
		MOD.Nest_SetBarGroupLock(bg, bp.locked)
		MOD.Nest_SetBarGroupAttribute(bg, "parentFrame", bp.parentFrame)
		MOD.Nest_SetBarGroupLabelFont(
			bg,
			media:Fetch("font", bp.labelFont),
			bp.labelFSize,
			bp.labelAlpha,
			bp.labelColor,
			bp.labelOutline,
		...
		MOD.Nest_SetBarGroupTimeFont(
			bg,
			media:Fetch("font", bp.timeFont),
			bp.timeFSize,
			bp.timeAlpha,
			bp.timeColor,
			bp.timeOutline,
		...
		MOD.Nest_SetBarGroupIconFont(
			bg,
			media:Fetch("font", bp.iconFont),
			bp.iconFSize,
			bp.iconAlpha,
			bp.iconColor,
			bp.iconOutline,
		...
		MOD.Nest_SetBarGroupTextures(
			bg,
			fgtexture,
			bp.fgAlpha,
			bgtexture,
			bp.bgAlpha,
			not bp.showNoDurationBackground,
			bp.fgSaturation,
			bp.fgBrightness,
			bp.bgSaturation,
			bp.bgBrightness
		)
		MOD.Nest_SetBarGroupVisibles(
			bg,
			not bp.hideIcon,
			not bp.hideClock,
			not bp.hideBar,
		...
		if bp.timelineTexture then
			bgtexture = media:Fetch("statusbar", bp.timelineTexture)
		else
			bgtexture = nil
		end
		if bp.timelineBorderTexture then
			fgtexture = (bp.timelineBorderTexture ~= "None") and media:Fetch("border", bp.timelineBorderTexture) or nil
		end
		MOD.Nest_SetBarGroupTimeline(
			bg,
			bp.timelineWidth,
			bp.timelineHeight,
			bp.timelineDuration,
			...
			bgtexture,
			bp.timelineAlpha,
			bp.timelineColor or gc,
			bp.timelineLabels or defaultLabels,
			fgtexture,
			bp.timelineBorderWidth,
			bp.timelineBorderOffset,
			bp.timelineBorderColor or gc
		)
		...
		if bp.sor == "T" then
			sf = "time"
		elseif bp.sor == "D" then
			sf = "duration"
		elseif bp.sor == "S" then
			sf = "start"
		end
		...
		if bp.segmentBars then
			MOD.Nest_SetBarGroupSegments(
				bg,
				bp.segmentCount,
				bp.segmentOverride,
				bp.segmentSpacing,
				bp.segmentHideEmpty,
				bp.segmentFadePartial,
				bp.segmentShrinkWidth,
				bp.segmentShrinkHeight,
				bp.segmentGradient,
				bp.segmentGradientAll,
				bp.segmentGradientStartColor,
				bp.segmentGradientEndColor,
				bp.segmentBorderColor,
				bp.segmentAdvanced,
				bp.segmentCurve,
				bp.segmentRotate,
				bp.segmentTexture
...
local function ShowStripe(bp, bg)
	local bgtexture = nil
	if bp.stripeTexture then
		bgtexture = media:Fetch("statusbar", bp.stripeTexture)
	end
	local sc = bp.stripeColor or gc
	if bp.stripeCheckCondition and bp.stripeCondition and MOD:CheckCondition(bp.stripeCondition) then
		sc = bp.stripeAltColor
	end
	local borderTexture = (bp.stripeBorderTexture ~= "None") and media:Fetch("border", bp.stripeBorderTexture) or nil
	MOD.Nest_SetBarGroupStripe(
		bg,
		bp.stripeFullWidth,
		bp.stripeWidth,
		bp.stripeHeight,
		...
		sc,
		borderTexture,
		bp.stripeBorderWidth,
		bp.stripeBorderOffset,
		bp.stripeBorderColor or gc
	)
end
			...
			if bg and bg.configuration then -- make sure already configured
				...
				elseif bp.anchor then
					...
					if IsOn(abp) and abp.enabled then -- make sure the anchor is actually around to attach
						MOD.Nest_SetRelativeAnchorPoint(
							bg,
							bp.anchor,
							nil,
							nil,
...
local function Bar_OnUpdate(bar)
	...
	if not tt then
		return
	end -- tooltipType set to nil suppresses tooltips
	...
	elseif (tt == "weapon") then
		local slotid = id
		if slotid == "MainHandSlot" then
			slotid = 16
		end
		if slotid == "SecondaryHandSlot" then
			slotid = 17
		end
	...
	elseif tt == "effect" then
		...
		if ect and ect.id then
			GameTooltip:SetSpellByID(ect.id)
		else
	...
	elseif tt == "notification" then
		...
		if ct then
			local c = ct[unit]
			if IsOn(c) and c.tooltip then
				GameTooltip:AddLine(MOD:GetConditionText(c.name), 1, 1, 1, true)
			end
		end
	...
	if IsControlKeyDown() then
		if spell then
			GameTooltip:AddLine("<Spell #" .. tonumber(spell) .. ">", 0, 1, 0.2, false)
		end
		if bat.listID then
			GameTooltip:AddLine("<List #" .. tonumber(bat.listID) .. ">", 0, 1, 0.2, false)
		end
	end
	if caster and (caster ~= "") then
		GameTooltip:AddLine(L["<Applied by "] .. caster .. ">", 0, 0.8, 1, false)
	end
	GameTooltip:Show()
end
...
local function Bar_AnchorTooltip(frame, tooltip, ttanchor)
	...
	elseif (ttanchor == "DEFAULT") and (GetCVar("UberTooltips") == "1") then
		...
		if not ttanchor or (ttanchor == "DEFAULT") then
			ttanchor = "ANCHOR_BOTTOMLEFT"
		else
			ttanchor = "ANCHOR_" .. ttanchor
		end
...
local function Bar_OnEnter(frame, bgName, barName, ttanchor)
	...
	if tt == "broker" then
		if type(db) == "table" then
			if
				db.tooltip and type(db.tooltip) == "table" and type(db.tooltip.SetText) == "function" and
					type(db.tooltip.Show) == "function"
			 then
				Bar_AnchorTooltip(frame, db.tooltip)
				if db.tooltiptext then
					db.tooltip:SetText(db.tooltiptext)
				end
				db.tooltip:Show()
...
local function Bar_OnClick(frame, bgName, barName, button)
	...
	elseif (tt == "header") and (button == "RightButton") then
		if IsShiftKeyDown() then
			MOD:RemoveMatchingTrackers(unit)
		else
			MOD:RemoveTrackers(unit)
		end
	elseif (tt == "broker") then
		if type(db) == "table" and type(db.OnClick) == "function" then
			db.OnClick(frame, button)
		end
	end
end
...
function MOD:TestBarGroup(bp)
	...
	if bg then
		...
		local timers = bp.testTimers or 0
		if timers == 0 then
			timers = 10
		end
		...
		if not found then
			for i = 1, timers do
				...
				if bar then
					...
					MOD.Nest_SetLabel(bar, L["Timer Bar"] .. " " .. i)
					MOD.Nest_SetIcon(bar, icon)
					MOD.Nest_SetCount(bar, i)
					MOD.Nest_StartTimer(bar, i * 5, 60, 60)
					MOD.Nest_SetCallbacks(bar, nil, Bar_OnEnter, Bar_OnLeave)
					bat.tooltipType = "test"
					bat.tooltipID = "timer"
					bat.tooltipUnit = i
					bat.updated = true
				end
			...
			for i = 1, static do
				...
				if bar then
					...
					local c = MOD.ColorPalette[testColors[(i % 10) + 1]]
					MOD.Nest_SetColors(bar, c.r, c.g, c.b, 1, c.r, c.g, c.b, 1, c.r, c.g, c.b, 1)
					MOD.Nest_SetLabel(bar, L["Test Bar"] .. " " .. i)
					MOD.Nest_SetIcon(bar, icon)
					MOD.Nest_SetCount(bar, i)
					MOD.Nest_SetCallbacks(bar, nil, Bar_OnEnter, Bar_OnLeave)
					bat.tooltipType = "test"
...
local function CheckTimeAndDuration(bp, timeLeft, duration)
	if (timeLeft == 0) and (duration == 0) then -- test for unlimited duration
		if not bp.showNoDuration then
			return false
		end
	else
		if bp.showNoDuration and bp.showOnlyNoDuration then
			return false
		end
		if bp.checkDuration and bp.filterDuration then
			if bp.minimumDuration then
				if duration < bp.filterDuration then
					return false
				end
			elseif duration >= bp.filterDuration then
				return false
			end
		end
		if bp.checkTimeLeft and bp.filterTimeLeft then
			if bp.minimumTimeLeft then
				if timeLeft < bp.filterTimeLeft then
					return false
				end
			elseif timeLeft >= bp.filterTimeLeft then
				return false
			end
		end
	end
...
local numberPatterns = {
	-- patterns for extracting up to 10 numbers from a string
	"(%d+%.?%d*)",
	"%d+%.?%d*%D+(%d+%.?%d*)",
	"%d+%.?%d*%D+%d+%.?%d*%D+(%d+%.?%d*)",
...
local function uncolor(s)
	local t = string.gsub(s, "|c........", "")
	t = string.gsub(t, "|r", "") -- remove color escape sequences from the text
	return t
end

-- Return a number found in a tooltip for auras and cooldowns
function MOD:GetTooltipNumber(ttType, ttID, ttUnit, ttOffset)
	if not ttOffset or ttOffset > #numberPatterns then
		ttOffset = 1
	end -- determine offset into numberPatterns
	local tt = nil
	if ttType == "buff" then
		tt = MOD:GetBuffTooltip()
		tt:SetUnitAura(ttUnit, ttID, "HELPFUL") -- fill in the tooltip for the buff
	elseif ttType == "debuff" then
		tt = MOD:GetBuffTooltip()
		tt:SetUnitAura(ttUnit, ttID, "HARMFUL") -- fill in the tooltip for the debuff
...
local function ShowCombatText(label, group, caption, icon, crit, ec)
	if MOD.suppress then
		return true
	end -- combat text is suppressed
	local t
	if group then
		t = string.format("%s [%s] %s", label, group, caption)
	else
		t = string.format("%s %s", label, caption)
	end

	if _G.MikSBT then
		_G.MikSBT.DisplayMessage(
			t,
			_G.MikSBT.DISPLAYTYPE_NOTIFICATION,
			crit,
			ec.r * 255,
			ec.g * 255,
			ec.b * 255,
...
local function PlaySoundMedia(soundMedia)
	if soundMedia and not MOD.suppress then -- make sure sound is not suppressed
		...
		if sound then
			PlaySoundFile(sound, Raven.db.global.SoundChannel)
		end
	end
end
...
local function UpdateBar(bp, vbp, bg, b, icon, timeLeft, duration, count, btype, ttType, ttID, ttUnit, ttCaster, isMine)
	local now = GetTime()
	if (duration or 0) > 0 then -- check if timer bar
		local elapsed = duration - timeLeft
		if (b.hide and (elapsed >= (b.delayTime or 0))) or (bp.hide and (elapsed >= (bp.delayTime or 0))) then
			return
		end
	end
	...
	if vbp.sor == "X" then
		barname = string.format("%05d ", b.sorder) .. barname
	end
	if bp.detectTotems then
		barname = b.uniqueID .. b.barLabel
	end -- use slot order for auto-generated totem bars
	...
	if vbp.setDuration then -- override with uniform duration for all bars in group (optionally ignore shorter bars)
		if not vbp.setOnlyLongDuration or (duration > vbp.uniformDuration) then
			maxTime = vbp.uniformDuration
		end
	end
	...
	if b.labelNumber then -- optionally get a number found in the tooltip and add it to the label
		...
		if string.find(label, "TT#") then -- special case to just show the number without rest of label
			if num and not b.startReady then
				label = num
			else
				label = ""
			end
			...
			if num then
				label = string.format("%s: %s", label, num)
			end
		end
	end
	...
	if b.colorBar then -- color may be overriden based on value of a condition
		...
		if result then
			if b.colorTrue and b.colorTrue.a > 0 then
				c = b.colorTrue
			end
		else
			if b.colorFalse and b.colorFalse.a > 0 then
				c = b.colorFalse
			end
		end
	end
	...
	if not (bp.showNoDuration and bp.showOnlyNoDuration) and not ((timeLeft == 0) and (duration == 0)) then -- bar with duration
		if bar and MOD.Nest_IsTimer(bar) then -- existing timer bar
			...
			if bar then
				MOD.Nest_StartTimer(bar, timeLeft, duration, maxTime)
				if b.ghost then
					bar.attributes.ghostDuration = b.endEffectTime or 5
				end
			end
		end
	elseif bp.showNoDuration or (bt == "Notification") or (bt == "Broker") or (bt == "Value") or b.enableReady then -- bars without duration
		if bar and MOD.Nest_IsTimer(bar) then
			MOD.Nest_DeleteBar(bg, bar)
			bar = nil
		end
		if not bar then
			bar = MOD.Nest_CreateBar(bg, barname)
			if bar and bt == "Notification" then
				MOD.Nest_SetFlash(bar, b.flash)
			end
		end
	end
	if bar then
		...
		if (bt == "Value") then
			if b.frequent then
				...
				frequentBars[b] = nil
			end -- save in the frequent bar update table
			if b.segmentCount then
				bat.segmentCount = b.segmentCount
			else
				bat.segmentCount = nil
			end -- this will override bar group setting
		end
		...
		elseif label then
			if vbp.casterLabels and ttCaster then
				label = "[" .. ttCaster .. "] " .. label
			end
			if vbp.spellLabels and b.spellID then
				label = label .. " [#" .. b.spellID .. "]"
			end
		...
		local tex = nil
		if b.action and MOD.db.global.SpellIcons[b.action] then
			tex = MOD:GetIcon(b.action)
		end -- check for override of the icon
		if tex then
			MOD.Nest_SetIcon(bar, tex)
		else
			MOD.Nest_SetIcon(bar, icon)
		end
		...
		elseif vbp.iconColors == "Debuffs" then -- special color for both buffs and debuffs
			...
			if dc then
				ibr, ibg, ibb, iba = dc.r, dc.g, dc.b, dc.a
			end
		...
		local desat =
			vbp.desaturate and (not vbp.desaturateFriend or (UnitExists("target") and UnitIsFriend("player", "target")))
		...
		if vbp.showTooltips and (vbp.combatTips or not MOD.status.inCombat) then
			bat.tooltipType = ttType
			bat.tooltipID = ttID
			bat.tooltipUnit = ttUnit -- tooltip info passed from bar source
			if vbp.spellTips then
				bat.tooltipSpell = b.spellID
			else
				bat.tooltipSpell = nil
			end -- for spell id in tooltip when control and alt keys are both down
		else
			bat.tooltipType = nil
			bat.tooltipID = nil
			bat.tooltipUnit = nil
			bat.tooltipSpell = nil -- tooltip info passed from bar source
		end
		bat.listID = b.listID -- for tooltip to show if found in a spell list
		if vbp.casterTips then
			bat.caster = ttCaster
		else
			bat.caster = nil
		end
		...
		if bt ~= "Notification" and bt ~= "Broker" and bt ~= "Value" then
			bat.saveBarGroup = vbp
			bat.saveBarType = bt
			bat.saveBarAction = b.action
		else
			bat.saveBarGroup = nil
			bat.saveBarType = nil
			bat.saveBarAction = nil
			if b.brokerVariable then -- support variable width brokers
				bat.minimumWidth = b.brokerMinimumWidth or 0
				bat.maximumWidth = b.brokerMaximumWidth or 1000
			else
				bat.minimumWidth = nil
				bat.maximumWidth = nil
			end
			...
			if bt == "Value" then
				if b.tickOffset then
					MOD.Nest_SetTick(bar, true, b.tickOffset, 1, 0, 0, 1)
					ticky = true
				end
				if b.value_r and b.value_g and b.value_b then
					if not b.hideValueColorForeground then
						MOD.Nest_SetForegroundColor(bar, b.value_r, b.value_g, b.value_b)
					end
					if b.showValueColorBackground then
						MOD.Nest_SetBackgroundColor(bar, b.value_r, b.value_g, b.value_b)
					end
				end
			end
		end
		...
		if sfxBar and (duration > (b.expireMinimum or 0)) then
			...
			if pt > expireBar then
				expireBar = pt
			end -- percent setting is minimum so only override if longer
			if not b.spellExpireTimes then -- for some reason, this is set to false when using spell's expire times
				...
				if st then
					expireBar = st
				end -- always override the expire time setting when set
			end
		end
		...
		local inStartBG =
			sfxBG and
			((elapsed >= (vbp.delayTime or 0)) and
				((vbp.startEffectTime == 0) or (elapsed <= ((vbp.delayTime or 0) + (vbp.startEffectTime or 5)))))
		local inStartBar =
			sfxBar and not b.startReady and
			((elapsed >= (b.delayTime or 0)) and
				((b.startEffectTime == 0) or (elapsed <= ((b.delayTime or 0) + (b.startEffectTime or 5)))))
		...
		if inStartBG and not vbp.selectAll then -- check filters for auto bar group start effects
			-- filters include: isPlayer, isPet, isDispel, isStealable, isPoison, isCurse, isMagic, isDisease, isEnrage
			inStartBG =
				(b.isPlayer and vbp.selectPlayer) or (b.isPet and vbp.selectPet) or (b.isDispel and vbp.selectDispel) or
				(b.isStealable and vbp.selectStealable) or
				(b.isPoison and vbp.selectPoison) or
				(b.isCurse and vbp.selectCurse) or
				(b.isMagic and vbp.selectMagic) or
				(b.isDisease and vbp.selectDisease) or
				(b.isEnrage and vbp.selectEnrage)
		end

		if elapsed == 0 then
			bar.shineStart = false -- reset flags for when bars are started or restarted
			bar.shineExpiring = false -- and for when bars are expiring
			bar.shineEnd = false -- and for when bars are nearly finished
			bar.shineReady = false -- and for ready bars
			bar.soundStart = false
			bar.soundExpire = false
			bar.soundEnd = false
			bar.soundReady = false -- and for playing sounds
			bar.combatStart = false
			bar.expireMSBT = false
			bar.combatEnd = false
			bar.combatReady = false -- and for combat text displays

			if not bar.timers then
				bar.timers = {}
			else
				table.wipe(bar.timers)
			end -- reset table used to set times for forced updates
			if sfxBG then
				t = now + (vbp.delayTime or 0)
				if t > now then
					bar.timers.inStartBG = t
				end
				if vbp.startEffectTime ~= 0 then
					t = t + (vbp.startEffectTime or 5)
					if t > now then
						bar.timers.effectBG = t
					end
				end
			end
			if sfxBar then
				t = now + (b.delayTime or 0)
				if t > now then
					bar.timers.inStartBar = t
				end
				if b.startEffectTime ~= 0 then
					t = t + (b.startEffectTime or 5)
					if t > now then
						bar.timers.effectBar = t
					end
				end
			end
			if ((duration or 0) > 0) and (timeLeft > 0) then
				if sfxBG and not inExpireBG then
					bar.timers.inExpireBG = expireBG and (now + duration - expireBG) or nil
				end
				if sfxBar and not inExpireBar then
					bar.timers.inExpireBar = expireBar and (now + duration - expireBar) or nil
				end
				t = now + duration
				if t > now then
					bar.timers.final = t
				...
				t = t - 0.62
				if t > now then
					bar.timers.inFinish = t
				end
			end
		end

		if inExpireBar and b.colorExpiring then -- change bar, label and time colors when expiring
			t = (b.spellExpireColors and MOD:GetExpireColor(b.action, b.spellID)) or b.expireColor or rc
			if t.a > 0 then
				MOD.Nest_SetForegroundColor(bar, t.r, t.g, t.b)
				if b.expireFGBG then
					MOD.Nest_SetBackgroundColor(bar, t.r, t.g, t.b)
				end
			end
			t = b.expireLabelColor or vc
			if t.a > 0 then
				MOD.Nest_SetLabelColor(bar, t.r, t.g, t.b)
			end
			t = b.expireTimeColor or vc
			if t.a > 0 then
				MOD.Nest_SetTimeColor(bar, t.r, t.g, t.b)
			end
		elseif inExpireBG and vbp.colorExpiring then
			t = (vbp.spellExpireColors and MOD:GetExpireColor(b.action, b.spellID)) or vbp.expireColor or rc
			if t.a > 0 then
				MOD.Nest_SetForegroundColor(bar, t.r, t.g, t.b)
				if vbp.expireFGBG then
					MOD.Nest_SetBackgroundColor(bar, t.r, t.g, t.b)
				end
			end
			t = vbp.expireLabelColor or vc
			if t.a > 0 then
				MOD.Nest_SetLabelColor(bar, t.r, t.g, t.b)
			end
			t = vbp.expireTimeColor or vc
			if t.a > 0 then
				MOD.Nest_SetTimeColor(bar, t.r, t.g, t.b)
			end
		else
		...
		if (duration or 0) > 0 then -- only timer bars of limited duration get ticks
			local offset = 0
			if sfxBar and b.colorExpiring then -- custom bar has precedence over bar group special effects if both are enabled
				...
				if bar.timers.inExpireBar and bar.timers.final then
					offset = bar.timers.final - bar.timers.inExpireBar
				end
				if t.a > 0 then
					MOD.Nest_SetTick(bar, true, offset, t.r, t.g, t.b, t.a)
					ticky = true
				end
			elseif sfxBG and vbp.colorExpiring then
				t = vbp.tickColor or vc -- show or hide tick mark for bar group
				if bar.timers.inExpireBG and bar.timers.final then
					offset = bar.timers.final - bar.timers.inExpireBG
				end
				if t.a > 0 then
					MOD.Nest_SetTick(bar, true, offset, t.r, t.g, t.b, t.a)
					ticky = true
		...
		if not ticky then
			MOD.Nest_SetTick(bar, false, 0)
		end

		bat.shineColor = wc
		bat.glowColor = wc -- first set defaults for customization options
		bat.flashPeriod = 1.2
		bat.flashPercent = 50
		...
		local shineExpiring =
			not bar.shineExpiring and ((vbp.shineExpiring and inExpireBG) or (b.shineExpiring and inExpireBar)) -- shine at expire time
		...
		MOD.Nest_SetShine(bar, shineStart or shineReady or (((duration or 0) > 0) and (shineExpiring or shineEnd)))
		if shineStart then
			bar.shineStart = true
		end
		if shineExpiring then
			bar.shineExpiring = true
		end
		if shineEnd then
			bar.shineEnd = true
		end
		...
		local isFlashing =
			(IsOn(b.flashBar) and (b.flashBar == MOD:CheckCondition(b.flashCondition))) or -- conditional flashing
			(vbp.flashStart and inStartBG) or
			(b.flashStart and inStartBar) or -- bar group or custom bar start effect
			(vbp.flashExpiring and inExpireBG) or
			(b.flashExpiring and inExpireBar) or -- bar group or custom bar expire effect
			(b.flashReady and b.startReady) -- ready bar flash option
		MOD.Nest_SetFlash(bar, isFlashing)

		local isGlowing =
			(IsOn(b.glowBar) and (b.glowBar == MOD:CheckCondition(b.glowCondition))) or -- conditional glow
			(vbp.glowStart and inStartBG) or
			(b.glowStart and inStartBar) or -- bar group or custom bar start effect
			(vbp.glowExpiring and inExpireBG) or
			(b.glowExpiring and inExpireBar) or -- bar group or custom bar expire effect
			(b.glowReady and inReadyBar) -- custom bar ready effect
		MOD.Nest_SetGlow(bar, isGlowing)

		local isDesaturated =
			(vbp.desatStart and inStartBG) or (b.desatStart and inStartBar) or -- bar group or custom bar start effect
			(vbp.desatExpiring and inExpireBG) or
			(b.desatExpiring and inExpireBar) or -- bar group or custom bar expire effect
			(b.desaturateReadyIcon and inReadyBar) -- custom bar ready effect
		if isDesaturated then
			bat.desaturate = true
		end -- desaturate icons (should not interfere with other desaturate settings)
		...
		if MOD.status.inCombat then
			alpha = (dft and vbp.combatAlpha or gd.combatAlpha)
		else
			alpha = (dft and vbp.alpha or gd.alpha)
		end
		if vbp.targetAlpha and ttUnit == "all" and b.group ~= UnitGUID("target") then
			alpha = alpha * vbp.targetAlpha
		end
		if IsOn(b.fadeBar) then -- conditional fade for bars is higher priority than bar group setting for delayed fade
			if (b.fadeBar == MOD:CheckCondition(b.fadeCondition)) and b.fadeAlpha then
				alpha = alpha * b.fadeAlpha
				isFaded = true
			end
		...
		elseif vbp.fade and vbp.fadeAlpha and not b.startReady then
			if inStartBG then
				alpha = alpha * vbp.fadeAlpha
				isFaded = true
			end
		end

		if not isFaded then -- fading is highest priority
			if b.startReady or (b.enableReady and count and (count >= 1)) then
				if b.readyAlpha then
					alpha = alpha * b.readyAlpha
				end -- adjust alpha for ready bars
			elseif b.normalAlpha then
				alpha = alpha * b.normalAlpha
			end
		end
		...
		if not bar.combatStart then
			if inStartBar and b.combatStart then
				bar.combatStart =
					ShowCombatText(label, tBar, L["started"], icon, b.combatCriticalStart, b.combatColorStart or rc)
			elseif inStartBG and vbp.combatStart then
				bar.combatStart =
					ShowCombatText(label, tBG, L["started"], icon, vbp.combatCriticalStart, vbp.combatColorStart or rc)
			end
		end
		...
		if not bar.combatEnd then
			if inFinishBar and b.combatEnd then
				bar.combatEnd =
					ShowCombatText(label, tBar, L["finished"], icon, b.combatCriticalEnd, b.combatColorEnd or rc)
			elseif inFinishBG and vbp.combatEnd then
				bar.combatEnd =
					ShowCombatText(label, tBG, L["finished"], icon, vbp.combatCriticalEnd, vbp.combatColorEnd or rc)
			end
		end
		if not bar.combatReady and inReadyBar and b.combatReady then
			bar.combatReady =
				ShowCombatText(label, tBar, L["ready"], icon, b.combatCriticalReady, b.combatColorReady or rc)
		end

		if not MOD.db.profile.muteSFX then -- make sure sounds are not muted
			if
				(inStartBG and (vbp.soundSpellStart or (vbp.soundAltStart ~= "None"))) or
					(inStartBar and (b.soundSpellStart or (b.soundAltStart ~= "None")))
			 then
				local sound, sp, replay, replayTime = nil, nil, false, 5 -- play start sound effect
				if vbp.soundSpellStart or b.soundSpellStart then
					sp = MOD:GetAssociatedSpellForBar(b)
					if sp then
						sound = MOD:GetSound(sp, b.spellID)
					end
				end
				if not sound and b.soundAltStart ~= "None" then
					sound = b.soundAltStart
				end
				if not sound and vbp.soundAltStart ~= "None" then
					sound = vbp.soundAltStart
				end
				if vbp.replay then
					replay = true
					replayTime = (vbp.replayTime or 5)
				elseif b.replay then
					replay = true
					replayTime = (b.replayTime or 5)
				end
			...
			if
				not bar.soundExpire and
					((inExpireBG and (vbp.soundSpellExpire or (vbp.soundAltExpire ~= "None"))) or
						(inExpireBar and (b.soundSpellExpire or (b.soundAltExpire ~= "None"))))
			 then
				local sound, sp = nil, nil -- play expire sound effect
				if vbp.soundSpellExpire or b.soundSpellExpire then
					sp = MOD:GetAssociatedSpellForBar(b)
					if sp then
						sound = MOD:GetSound(sp, b.spellID)
					end
				end
				if not sound and b.soundAltExpire ~= "None" then
					sound = b.soundAltExpire
				end
				if not sound and vbp.soundAltExpire ~= "None" then
					sound = vbp.soundAltExpire
				end
				if sound then
					PlaySoundMedia(sound)
			...
			if
				not bar.soundEnd and
					((inFinishBG and (vbp.soundSpellEnd or (vbp.soundAltEnd ~= "None"))) or
						(inFinishBar and (b.soundSpellEnd or (b.soundAltEnd ~= "None"))))
			 then
				local sound, sp = nil, nil -- play finish sound effect
				if vbp.soundSpellEnd or b.soundSpellEnd then
					sp = MOD:GetAssociatedSpellForBar(b)
					if sp then
						sound = MOD:GetSound(sp, b.spellID)
					end
				end
				if not sound and b.soundAltEnd ~= "None" then
					sound = b.soundAltEnd
				end
				if not sound and vbp.soundAltEnd ~= "None" then
					sound = vbp.soundAltEnd
				end
				if sound then
					PlaySoundMedia(sound)
					bar.soundEnd = true
			...
			if not bar.soundReady and (inReadyBar and (b.soundSpellReady or (b.soundAltReady ~= "None"))) then
				...
				if b.soundSpellReady then
					sp = MOD:GetAssociatedSpellForBar(b)
					if sp then
						sound = MOD:GetSound(sp, b.spellID)
					end
				end
				if not sound and b.soundAltReady ~= "None" then
					sound = b.soundAltReady
				end
				if sound then
					PlaySoundMedia(sound)
					bar.soundReady = true
...
function MOD:CheckCastBy(caster, cb)
	...
	return ((cb == "player") and isMine) or (cb == "anyone") or ((cb == "pet") and isPet) or
		((cb == "other") and not isOurs) or
		((cb == "ours") and isOurs) or
		((cb == "nother") and not (isOurs or isTarget)) or
		((cb == "target") and isTarget) or
		((cb == "focus") and isFocus)
end

-- Check if an action is in the associated filter bar group
local function CheckFilterBarGroup(bgname, btype, action, value)
	if not bgname or not action then
		return false
	end
	...
	if IsOn(bg) then
		if bg.auto then -- auto bar groups look in the filter list (doesn't matter if black list or white list)
			if btype == "Buff" then
				if (bg.filterBuff or bg.showBuff) and bg.filterBuffList and bg.filterBuffList[action] then
					return true
				end
			elseif btype == "Debuff" then
				if (bg.filterDebuff or bg.showDebuff) and bg.filterDebuffList and bg.filterDebuffList[action] then
					return true
				end
			elseif btype == "Cooldown" then
				if (bg.filterCooldown or bg.showCooldown) and bg.filterCooldownList and bg.filterCooldownList[action] then
					return true
				end
			end
			...
			if v == value then
				return true
			end -- for auras this is which unit is being monitored
		end
	end
	return false
end
...
local function DetectNewBuffs(unit, n, aura, isBuff, bp, vbp, bg)
	...
	if bp.showBuff or bp.filterBuff then -- check black lists and white lists
		...
		if not listed then -- not found in the bar group's filter list, so check spell lists
			...
			if bp.filterBuffSpells and bp.filterBuffTable then
				spellList = MOD.db.global.SpellLists[bp.filterBuffTable]
			end
			if spellList then
				listed = spellList[n] or (spellNum and spellList[spellNum])
			end -- check first spell list, if specified
			if not listed then
				spellList = nil -- check second spell list, if specified
				if bp.filterBuffSpells2 and bp.filterBuffTable2 then
					spellList = MOD.db.global.SpellLists[bp.filterBuffTable2]
				end
				if spellList then
					listed = spellList[n] or (spellNum and spellList[spellNum])
				end
				if not listed then
					...
					if bp.filterBuffSpells3 and bp.filterBuffTable3 then
						spellList = MOD.db.global.SpellLists[bp.filterBuffTable3]
					end
					if spellList then
						listed = spellList[n] or (spellNum and spellList[spellNum])
					end
					if not listed then
						...
						if bp.filterBuffSpells4 and bp.filterBuffTable4 then
							spellList = MOD.db.global.SpellLists[bp.filterBuffTable4]
						end
						if spellList then
							listed = spellList[n] or (spellNum and spellList[spellNum])
						end
						if not listed then
							...
							if bp.filterBuffSpells5 and bp.filterBuffTable5 then
								spellList = MOD.db.global.SpellLists[bp.filterBuffTable5]
							end
							if spellList then
								listed = spellList[n] or (spellNum and spellList[spellNum])
							end
			...
			if listed then
				listID = listed
			end -- if found in a spell list then save for tooltip display
		end
		if (bp.filterBuff and listed) or (bp.showBuff and not listed) then
			return
		end
	end
	if bp.filterBuffBars and CheckFilterBarGroup(bp.filterBuffBarGroup, "Buff", n, bp.detectBuffsMonitor) then
		return
	end -- check if in filter bar group
	...
	if (ttype == "Totem") and not bp.includeTotems then
		return
	end -- check if including totems
	if tt == "minion" and tsteal then
		label = label .. MinionEnergy(tsteal)
		tsteal = nil
	end -- -- add wild imp energy
	...
	local isOther =
		not isStealable and not isCastable and not isMagic and not isEffect and not isWeapon and not isEnrage and
		not isTracking and
		not isResource and
		not isMount and
		not isTabard
	...
	if checkAll then
		id = aura[18]
		gname = aura[19]
	end -- these fields are only valid if unit == "all"

	local includeTypes =
		not bp.detectBuffTypes or (bp.detectStealable and isStealable) or (bp.detectCastable and isCastable) or
		(bp.detectEnrageBuffs and isEnrage) or
		(bp.detectMagicBuffs and isMagic) or
		(bp.detectEffectBuffs and isEffect) or
		(bp.detectAlertBuffs and isAlert) or
		(bp.detectWeaponBuffs and isWeapon) or
		(bp.detectTracking and isTracking) or
		(bp.detectResources and isResource) or
		(bp.detectMountBuffs and isMount) or
		(bp.detectTabardBuffs and isTabard) or
		(bp.detectMinionBuffs and isMinion) or
		(bp.detectOtherBuffs and isOther)
	local excludeTypes =
		not bp.excludeBuffTypes or
		not ((bp.excludeStealable and isStealable) or (bp.excludeCastable and isCastable) or
			(bp.excludeEnrageBuffs and isEnrage) or
			(bp.excludeMagicBuffs and isMagic) or
			(bp.excludeEffectBuffs and isEffect) or
			(bp.excludeAlertBuffs and isAlert) or
			(bp.excludeWeaponBuffs and isWeapon) or
			(bp.excludeTracking and isTracking) or
			(bp.excludeResources and isResource) or
			(bp.excludeMountBuffs and isMount) or
			(bp.excludeTabardBuffs and isTabard) or
			(bp.excludeMinionBuffs and isMinion) or
			(bp.excludeOtherBuffs and isOther))
	if
		((checkAll and not (bp.noPlayerBuffs and (id == UnitGUID("player"))) and
			not (bp.noPetBuffs and (id == UnitGUID("pet"))) and
			not (bp.noTargetBuffs and (id == UnitGUID("target"))) and
			not (bp.noFocusBuffs and (id == UnitGUID("focus")))) or
			(not checkAll and not (bp.noPlayerBuffs and UnitIsUnit(unit, "player")) and
				not (bp.noPetBuffs and UnitIsUnit(unit, "pet")) and
				not (bp.noTargetBuffs and UnitIsUnit(unit, "target")) and
				not (bp.noFocusBuffs and UnitIsUnit(unit, "focus")) and
				MOD:CheckCastBy(tc, bp.detectBuffsCastBy))) and
			CheckTimeAndDuration(bp, aura[2], aura[5]) and
			includeTypes and
			excludeTypes
	 then
		...
		if unit == "all" then
			tag = tag .. ":" .. id
			if vbp.noHeaders then
				label =
					(vbp.noLabels and "" or (label .. (vbp.noTargets and "" or " - "))) ..
					(vbp.noTargets and "" or gname)
			end
		end
		if isEffect and ta then
			local ect = MOD.db.global.SpellEffects[ta]
			if ect and ect.label then
				label = label .. " |cFF7adbf2[" .. (UnitName(tc) or tc) .. "]|r"
			end -- prefer caster's name if available
		end
		if isAlert then
			b.barColor = aura[18]
			if aura[19] then
...
local function DetectNewDebuffs(unit, n, aura, isBuff, bp, vbp, bg)
	...
	if bp.showDebuff or bp.filterDebuff then -- check black lists and white lists
		...
		if not listed then -- not found in the bar group's filter list, so check spell lists
			local spellList = nil -- check the first spell list, if specified
			if bp.filterDebuffSpells and bp.filterDebuffTable then
				spellList = MOD.db.global.SpellLists[bp.filterDebuffTable]
			end
			if spellList then
				listed = spellList[n] or (spellNum and spellList[spellNum])
			end -- check first spell list, if specified
			if not listed then
				spellList = nil -- check second spell list, if specified
				if bp.filterDebuffSpells2 and bp.filterDebuffTable2 then
					spellList = MOD.db.global.SpellLists[bp.filterDebuffTable2]
				end
				if spellList then
					listed = spellList[n] or (spellNum and spellList[spellNum])
				end
				if not listed then
					spellList = nil -- check third spell list, if specified
					if bp.filterDebuffSpells3 and bp.filterDebuffTable3 then
						spellList = MOD.db.global.SpellLists[bp.filterDebuffTable3]
					end
					if spellList then
						listed = spellList[n] or (spellNum and spellList[spellNum])
					end
					if not listed then
						spellList = nil -- check fourth spell list, if specified
						if bp.filterDebuffSpells4 and bp.filterDebuffTable4 then
							spellList = MOD.db.global.SpellLists[bp.filterDebuffTable4]
						end
						if spellList then
							listed = spellList[n] or (spellNum and spellList[spellNum])
						end
						if not listed then
							spellList = nil -- check fifth spell list, if specified
							if bp.filterDebuffSpells5 and bp.filterDebuffTable5 then
								spellList = MOD.db.global.SpellLists[bp.filterDebuffTable5]
							end
							if spellList then
								listed = spellList[n] or (spellNum and spellList[spellNum])
							end
		...
		if (bp.filterDebuff and listed) or (bp.showDebuff and not listed) then
			return
		end
	end
	if bp.filterDebuffBars and CheckFilterBarGroup(bp.filterDebuffBarGroup, "Debuff", n, bp.detectDebuffsMonitor) then
		return
	end -- check if in filter bar group
	...
	local isEffect = (tt == "effect")
	local isAlert = (tt == "alert")
	local isPoison, isCurse, isMagic, isDisease =
		(aura[4] == "Poison"),
		(aura[4] == "Curse"),
		(aura[4] == "Magic"),
		(aura[4] == "Disease")
	local isOther =
		not isEffect and not isPoison and not isCurse and not isMagic and not isDisease and not isDispel and
		not isInflict
	local isMine = (tc == "player")
	local id, gname = aura[18], aura[19]
	local checkAll = (unit == "all")
	local includeTypes =
		not bp.filterDebuffTypes or (bp.detectDispellable and isDispel) or (bp.detectInflictable and isInflict) or
		(bp.detectPoison and isPoison) or
		(bp.detectCurse and isCurse) or
		(bp.detectMagic and isMagic) or
		(bp.detectDisease and isDisease) or
		(bp.detectEffectDebuffs and isEffect) or
		(bp.detectAlertDebuffs and isAlert) or
		(bp.detectOtherDebuffs and isOther)
	local excludeTypes =
		not bp.excludeDebuffTypes or
		not ((bp.excludeDispellable and isDispel) or (bp.excludeInflictable and isInflict) or
			(bp.excludePoison and isPoison) or
			(bp.excludeCurse and isCurse) or
			(bp.excludeMagic and isMagic) or
			(bp.excludeDisease and isDisease) or
			(bp.excludeEffectDebuffs and isEffect) or
			(bp.excludeAlertDebuffs and isAlert) or
			(bp.excludeOtherDebuffs and isOther))
	if
		((checkAll and not (bp.noPlayerDebuffs and (id == UnitGUID("player"))) and
			not (bp.noPetDebuffs and (id == UnitGUID("pet"))) and
			not (bp.noTargetDebuffs and (id == UnitGUID("target"))) and
			not (bp.noFocusDebuffs and (id == UnitGUID("focus")))) or
			(not checkAll and not (bp.noPlayerDebuffs and UnitIsUnit(unit, "player")) and
				not (bp.noPetDebuffs and UnitIsUnit(unit, "pet")) and
				not (bp.noTargetDebuffs and UnitIsUnit(unit, "target")) and
				not (bp.noFocusDebuffs and UnitIsUnit(unit, "focus")) and
				MOD:CheckCastBy(tc, bp.detectDebuffsCastBy))) and
			CheckTimeAndDuration(bp, aura[2], aura[5]) and
			includeTypes and
			excludeTypes
	 then
...
local function CheckCooldownType(cd, bp)
	...
	if (t == "spell") or (t == "spell id") then
		other = false
		if bp.detectSpellCooldowns then
			return true
		end
	elseif (t == "inventory") and ((s == 13) or (s == 14)) then
		other = false
		if bp.detectTrinketCooldowns then
			return true
		end
	elseif (t == "internal") then
		other = false
		if bp.detectInternalCooldowns then
			return true
		end
	elseif t == "effect" then
		other = false
		if bp.detectSpellEffectCooldowns then
			return true
		end
	elseif t == "alert" then
		other = false
		if bp.detectSpellAlertCooldowns then
			return true
		end
	elseif t == "text" then -- might be a potion or elixir
		if (s == "Shared Potion Cooldown") or (s == "Shared Elixir Cooldown") then
			other = false
			if bp.detectPotionCooldowns then
				return true
			end
		end
	end
	if other and bp.detectOtherCooldowns then
		return true
	end
	return false
end
...
local function CheckSharedCooldowns(b, bp)
	...
	if MOD.myClass == "WARLOCK" then
		if bp.detectSharedGrimoires then
			if id == 111895 or id == 111896 or id == 111897 or id == 111898 then
				return false
			end
			if id == 111859 then
				b.barLabel = GetSpellInfo(216187)
				return true
			end
		end
		if bp.detectSharedInfernals then
			...
			if id == 1122 then
				b.barLabel = L["Summon Infernal/Doomguard"]
				return true
			end
		end
	end
...
local function AutoRuneBars(bp, vbp, bg)
	...
	for i = 1, 6 do
		...
		b.action = L["Rune"]
		b.spellID = nil
		b.barLabel = runeSlotPrefix[i] .. b.action
		b.barType = "Cooldown"
		b.uniqueID = "Cooldown"
		b.group = nil
...
local function DetectNewCooldowns(n, cd, bp, vbp, bg)
	...
	if bp.showCooldown or bp.filterCooldown then -- check black lists and white lists
		...
		local listed =
			bp.filterCooldownList and (bp.filterCooldownList[n] or (spellNum and bp.filterCooldownList[spellNum]))
		if not listed then -- not found in the bar group's filter list, so check up spell lists
			local spellList = nil -- check the first spell list, if specified
			if bp.filterCooldownSpells and bp.filterCooldownTable then
				spellList = MOD.db.global.SpellLists[bp.filterCooldownTable]
			end
			if spellList then
				listed = spellList[n] or (spellNum and spellList[spellNum])
			end -- check first spell list, if specified
			if not listed then
				spellList = nil -- check second spell list, if specified
				if bp.filterCooldownSpells2 and bp.filterCooldownTable2 then
					spellList = MOD.db.global.SpellLists[bp.filterCooldownTable2]
				end
				if spellList then
					listed = spellList[n] or (spellNum and spellList[spellNum])
				end
				if not listed then
					spellList = nil -- check third spell list, if specified
					if bp.filterCooldownSpells3 and bp.filterCooldownTable3 then
						spellList = MOD.db.global.SpellLists[bp.filterCooldownTable3]
					end
					if spellList then
						listed = spellList[n] or (spellNum and spellList[spellNum])
					end
					if not listed then
						spellList = nil -- check fourth spell list, if specified
						if bp.filterCooldownSpells4 and bp.filterCooldownTable4 then
							spellList = MOD.db.global.SpellLists[bp.filterCooldownTable4]
						end
						if spellList then
							listed = spellList[n] or (spellNum and spellList[spellNum])
						end
						if not listed then
							spellList = nil -- check fifth spell list, if specified
							if bp.filterCooldownSpells5 and bp.filterCooldownTable5 then
								spellList = MOD.db.global.SpellLists[bp.filterCooldownTable5]
							end
							if spellList then
								listed = spellList[n] or (spellNum and spellList[spellNum])
							end
		...
		if (bp.filterCooldown and listed) or (bp.showCooldown and not listed) then
			return
		end
	end
	if bp.filterCooldownBars and CheckFilterBarGroup(bp.filterCooldownBarGroup, "Cooldown", n, true) then
		return
	end -- check if in filter bar group
	if MOD:CheckCastBy(cd[7], bp.detectCooldownsBy) and CheckCooldownType(cd, bp) and CheckTimeAndDuration(bp, cd[1], cd[4]) then
		...
		if (cd[11] == "alert") then -- TODO: CHECKME
			b.barColor = cd[10]
			if cd[11] then
				label = cd[11]
			end
...
local function CheckShow(bp)
	...
	if
		InCinematic() or (UnitOnTaxi("player") and not bp.showOnTaxi) or (pst == "solo" and not bp.showSolo) or
			(pst == "party" and not bp.showParty) or
			(pst == "raid" and not bp.showRaid) or
			(stat.inCombat and not bp.showCombat) or
			(not stat.inCombat and not bp.showOOC) or
			(not MOD.db.profile.hideBlizz and not bp.showBlizz) or
			(MOD.db.profile.hideBlizz and not bp.showNotBlizz) or
			(stat.isResting and not bp.showResting) or
			(stat.isStealthed and not bp.showStealth) or
			(stat.isMounted and not bp.showMounted) or
			(stat.inVehicle and not bp.showVehicle) or
			(stat.targetEnemy and not bp.showEnemy) or
			(stat.targetFriend and not bp.showFriend) or
			(stat.targetNeutral and not bp.showNeutral) or
			(stat.inInstance and not bp.showInstance) or
			(not stat.inInstance and not bp.showNotInstance) or
			(stat.inArena and not bp.showArena) or
			(stat.inBattleground and not bp.showBattleground) or
			(UnitIsUnit("focus", "target") and not bp.showFocusTarget) or
			(bp.showClasses and bp.showClasses[MOD.myClass]) or
			(bp.showSpecialization and bp.showSpecialization ~= "" and
				not MOD.CheckSpec(bp.showSpecialization, bp.specializationList)) or
			(bp.checkCondition and IsOn(bp.condition) and not MOD:CheckCondition(bp.condition))
	 then
		return false
	end
	return true
...
local function UpdateBarGroupBars(bp, vbp, bg)
	if CheckShow(bp) then
		if bp.auto then -- if auto bar group then detect new auras and cooldowns
			if bp.detectBuffs then
				MOD:IterateAuras(bp.detectAllBuffs and "all" or bp.detectBuffsMonitor, DetectNewBuffs, true, bp, vbp, bg)
			end
			if bp.detectDebuffs then
				MOD:IterateAuras(bp.detectAllDebuffs and "all" or bp.detectDebuffsMonitor, DetectNewDebuffs, false, bp, vbp, bg)
			end
			if bp.detectCooldowns then
				MOD:IterateCooldowns(DetectNewCooldowns, bp, vbp, bg)
			end
			if bp.detectRuneCooldowns then
				AutoRuneBars(bp, vbp, bg)
			end
			if bp.detectTotems then
				AutoTotemBars(bp, vbp, bg)
			end
			...
			if (not vbp.noHeaders or vbp.headerGaps) and ((bp.detectBuffs and bp.detectAllBuffs) or (bp.detectDebuffs and bp.detectAllDebuffs)) then -- add group headers, if necessary
				...
				for _, bar in pairs(MOD.Nest_GetBars(bg)) do
					...
					if id and gname and bar.attributes.updated then
						groupIDs[id] = gname
					end
				end
				for id, name in pairs(groupIDs) do -- create the header bars (these get added even if just want the header gaps)
					...
					if rti then
						label = prefixRaidTargetIcon .. rti .. ":0|t " .. name
					end
					table.wipe(b)
					b.enableBar = true
					b.sorder = 0
			...
			for _, bar in pairs(bp.bars) do -- iterate over each bar in the bar group
				...
				local specCheck =
					not (bar.showSpecialization and bar.showSpecialization ~= "" and
					not MOD.CheckSpec(bar.showSpecialization, bar.specializationList))
				...
				if bar.enableBar and hideCheck and classCheck and specCheck then
					...
					if (bt == "Buff") or (bt == "Debuff") then
						...
						if #auraList > 0 then
							for _, aura in pairs(auraList) do
								...
								local mon =
									((cb == "player") and isMine) or ((cb == "pet") and isPet) or
									((cb == "other") and not isMine) or
									(cb == "anyone")
								if mon and CheckTimeAndDuration(bp, aura[2], aura[5]) then
									if aura[11] == "minion" and aura[7] then
										bar.barLabel = bar.barLabel .. MinionEnergy(aura[7])
									end -- add wild imp energy
									count = count + 1
									if count > 1 then
						...
						if not found and bar.enableReady and aname then -- see if need aura ready bar
							if (bar.readyNotUsable or MOD:CheckSpellStatus(aname, true, true) or IsUsableItem(aname)) then -- check if really usable
								if not bar.readyTime then
									bar.readyTime = 0
								end
								if bar.readyTime == 0 then
									bar.startReady = nil
								end
								if not bar.startReady or ((GetTime() - bar.startReady) < bar.readyTime) then
									if not bar.startReady then
										bar.startReady = GetTime()
									end
					...
					elseif bt == "Cooldown" then
						...
						if
							not found and bar.enableReady and aname and
								(bar.readyNotUsable or MOD:CheckSpellStatus(aname, true) or IsUsableItem(aname))
						 then -- see if need cooldown ready bar
							if not bar.readyTime then
								bar.readyTime = 0
							end
							if bar.readyTime == 0 then
							...
							if not bar.startReady or ((GetTime() - bar.startReady) < bar.readyTime) then
								...
								if not iname then
									icon = MOD:GetIcon(aname)
								end
								UpdateBar(bp, vbp, bg, bar, icon, 0, 0, nil, nil, "text", aname, nil, nil, true)
							end
						end
										...
										if bp.segmentBars then -- check if segmented bar group
											...
											if bar.adjustSegments then -- see if the bar is allowed to adjust settings
												if bp.segmentOverride then -- see if bar group allows override with bar settings
													...
													if maxValue < bp.segmentCount then
														maxValue = bp.segmentCount
													end
												end
											end
										end
										if not bar.hideValueIcon then
											icon = valueIcon
										end
										local noLabel =
											(bar.hideEmptyLabel and isEmpty) or (bar.hideFullLabel and isFull)
										if noLabel then
											valueLabel = ""
										elseif bar.hideValueLabel then
											valueLabel = nil
										end
										local noText =
											bar.hideFormatText or (bar.hideEmptyText and isEmpty) or
											(bar.hideFullText and isFull)
										if noText then
											valueText = ""
										elseif bar.hideValueText then
											valueText = nil
										end
										bar.value = value
						...
						if db then
							...
							if bar.hideText or not s then
								if bar.brokerLabel then
									s = db.label
									if not s then
										s = MOD.LibLDB:GetNameByDataObject(db)
									end
									if bar.recolorText then
										s = uncolor(s)
									end
								else
									s = ""
								...
								if bar.brokerLabel then
									...
									if bar.barText == "" then
										...
										bar.barText = s .. ": " .. bar.barText
									end
								end
							end
							s = db.value or db.text
							if s then
								...
								if n then
									...
									elseif bar.brokerMaximum and bar.brokerMaxValue then
										...
										if m > 0 then
											...
											bar.maxValue = m
											bar.valueText =
												(bar.recolorText and "Max: " or "|cFF7adbf2Max:|r ") ..
												tostring(bar.maxValue)
										end
									end
								end
							end
...
-- Look for expired timer bars and show final effects (ghost bars)
local function UpdateFinalEffects(bp, bg)
	...
	for _, bar in pairs(MOD.Nest_GetBars(bg)) do -- first, find any elapsed timer bars and build table of active spells
		if MOD.Nest_IsTimer(bar) then
			local bat = bar.attributes
			if bat.updated then
				if bar.timeLeft == 0 then
					bat.updated = false
				elseif bat.tooltipSpell then
					activeSpells[bat.tooltipSpell] = true
				end
			end
		end
	end

	for _, bar in pairs(MOD.Nest_GetBars(bg)) do -- second, create ghost bars for expired timer bars
		if MOD.Nest_IsTimer(bar) then
			local bat = bar.attributes
			if not bp.auto then -- if custom bar group check for individual bar ghost option (overrides bar group option)
				if bat.ghostDuration and not bat.updated and not activeSpells[bat.tooltipSpell or 0] then -- check if candidate for ghost bar
					if not bat.ghostTime then
						MOD.Nest_SetCount(bar, nil)
						bat.ghostTime = now + bat.ghostDuration
					end
					if bat.ghostTime >= now then
						bat.updated = true
					end
				end
			elseif bp.ghost then -- for auto bars, if ghost enabled, any elapsed timer is a potential ghost bar
				if not bat.updated and not activeSpells[bat.tooltipSpell or 0] then
					if not bat.ghostTime then
						MOD.Nest_SetCount(bar, nil)
						bat.ghostTime = now + (bp.endEffectTime or 5)
					end
					if bat.ghostTime >= now then
						bat.updated = true
					end
	...
	for b, bar in pairs(frequentBars) do -- special table with value bars flagged in previous update
		...
		if name then
			...
			if f then
				...
				local status, value, maxValue, valueText, valueLabel =
					f(unit and b.monitor or nil, fmt, b.spell, b.optionalText)
				if status then
					local isEmpty, isFull = false, false
					if value and maxValue then
						isEmpty = (value == 0) and (maxValue > 0)
					...
					if (value ~= b.value) and (value <= b.maxValue) then
						...
						if noLabel then
							valueLabel = ""
						elseif b.hideValueLabel then
							valueLabel = nil
						end
						local noText = b.hideFormatText or (b.hideEmptyText and isEmpty) or (b.hideFullText and isFull)
						if noText then
							valueText = ""
						elseif b.hideValueText then
							valueText = nil
						end
						b.value = value
						b.valueText = valueText
						b.valueLabel = valueLabel -- never update maxValue during refreshes
