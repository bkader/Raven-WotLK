...
local overlayDefaults = {
	-- backdrop initialization for overlay grid
...
function MOD:SetInCombatBarDefaults()
	...
	p.offsetX = 0.5
	p.offsetY = 0.98
	p.anchorTips = "DEFAULT"
	p.flashExpiring = false
	p.flashTime = 5
	p.mouseoverDetect = false
	p.mouseoverAlpha = 0.5
	p.noBorder = false
	p.anchorFrame = ""
	p.anchorPoint = "CENTER"
	p.anchorX = 0
	p.anchorY = 0
	for k in pairs(inCombatBarTemplate) do
		g[k] = p[k]
	end
end
...
local function OverlayTooltipUpdate()
	if MOD.tooltipOverlay and MOD.Overlay_OnEnter then
		MOD.Overlay_OnEnter(MOD.tooltipOverlay)
	end
end
...
function MOD:UpdateInCombatBar()
	if not InCombatLockdown() then -- only update settings when not in combat
		...
		for k, v in pairs(MOD.db.profile.InCombatBar) do
			if p[k] ~= v then
				update = true
				p[k] = v
			end
...
local function Overlay_OnEnter(b)
	if b.aura_id then
		...
		elseif b.aura_tt == "buff" then
			if not UnitAura("player", b.aura_id, "HELPFUL") then
				return
			end
		...
		elseif b.aura_tt == "spell name" then
			...
			if #auraList > 0 then
				local aura = auraList[1]
				GameTooltip:SetUnitAura("player", aura[12], "HELPFUL")
			end
		end
		if IsControlKeyDown() then
			if b.aura_spell then
				GameTooltip:AddLine("<Spell #" .. tonumber(b.aura_spell) .. ">", 0, 1, 0.2, false)
			end
			if b.aura_list then
				GameTooltip:AddLine("<List #" .. tonumber(b.aura_list) .. ">", 0, 1, 0.2, false)
			end
		end
		if b.aura_caster and (b.aura_caster ~= "") then
			GameTooltip:AddLine(L["<Applied by "] .. b.aura_caster .. ">", 0, 0.8, 1, false)
		end
		GameTooltip:Show()
...
local function InCombatBar_OnMouseDown(anchor, button)
	...
	if g and g.opt then
		...
		if scale and (button == "LeftButton") and not IsModifierKeyDown() and g.initialized and not g.lock then
			g.startX = g.frame:GetLeft() * scale
			g.startY = g.frame:GetTop() * scale
			g.moving = true
	...
	if g and g.opt then
		...
		if scale and g.initialized and g.moving then
			...
			if g.startX ~= x or g.startY ~= y then
				...
				if g.anchor == "TOPRIGHT" then
					x = x + fw
				elseif g.anchor == "BOTTOMRIGHT" then
					x = x + fw
					y = y - fh
				end
				g.opt.offsetX = x / displayWidth
				g.opt.offsetY = y / displayHeight
...
function MOD:SetInCombatBar(enable, lock, list, opt)
	...
	if not g.initialized then -- first time, create base frame for holding secure buttons and mask frame for overlays
		...
		g.frame:SetClampedToScreen(true)
	...
	if g.count > 0 then
		...
		if opt.anchorFrame and GetClickFrame(opt.anchorFrame) then -- use relative positioning
			...
			g.frame:ClearAllPoints()
			g.frame:SetPoint(g.anchor, UIParent, "BOTTOMLEFT", dx, dy)
		end
		if lock then
			g.frame:Hide()
		else
		...
		g.mask:SetFrameStrata("MEDIUM")
		g.mask:SetAlpha(opt.alpha)
		g.mask:SetScale(opt.scale)
		g.mask:SetSize(w, h)
		g.mask:ClearAllPoints()
		g.mask:SetAllPoints(g.frame)
		g.mask:SetFrameLevel(g.frame:GetFrameLevel() + 5)

		for i = 1, g.count do
			local id, tt = list[i], "buff"
			local b = g.secureButtons[i]
			b.aura_id = id
			b.aura_tt = "spell name"
			b.tooltipAnchor = opt.anchorTips
			b:SetAttribute("type2", "macro")
			b:SetAttribute("macrotext2", "/cancelaura " .. id)
			b:SetSize(opt.size, opt.size)
			b:SetScale(opt.scale)
			local dist = i - 1
			if (opt.layout and not opt.direction) or (not opt.layout and opt.direction) then
				dist = g.count - i
			end
			...
			b:ClearAllPoints()
			b:SetPoint("TOPLEFT", g.frame, "TOPLEFT", x, -y)
			b:SetFrameLevel(g.frame:GetFrameLevel() + 1)
			if lock then
				b:SetAlpha(0)
			else
				b:SetAlpha(opt.alpha)
			end
			b:Show()
			local v = g.overlays[i]
			v.buffName = id
			v.container:SetSize(opt.size, opt.size)
			v.container:SetPoint("TOPLEFT", g.mask, "TOPLEFT", x, -y)
			v.highlight:ClearAllPoints()
			v.highlight:SetPoint("CENTER", v.container, "CENTER")
			v.backdrop:SetAllPoints(v.container)
		end
	...
	if g and g.opt and g.initialized then -- make sure have valid layout
		...
		for i = 1, g.count do
			...
			if not g.hidden and (g.opt.ooc or InCombatLockdown()) then -- check if in combat or have out-of-combat enabled
				buff, icon, _, _, _, duration, expires = MOD.UnitAuraSpellName("player", id, "HELPFUL")
			end
			if buff then
				v.highlight:SetTexture(icon)
				v.highlight:Show()
				v.container:EnableMouse(false)
				local w = g.opt.size
				if g.opt.noBorder then -- enable or hide optional default border
					v.backdrop:Hide()
					v.highlight:SetTexCoord(0, 1, 0, 1)
				else
					v.backdrop:Show()
					v.highlight:SetTexCoord(0.06, 0.94, 0.06, 0.94)
					w = w * 0.88
				end
				...
				if duration and duration > 0 then
					...
					if flashing then
						alpha = MOD.Nest_FlashAlpha(1, 0.5, 1.2)
					end
				end
				if g.opt.mouseoverDetect and not g.mask:IsMouseOver(2, -2, -2, 2) then
					alpha = alpha * (g.opt.mouseoverAlpha or 0.5)
				end
				...
				v.anim:Stop()
				v.startTime = nil
				v.container:EnableMouse(not g.hidden)
			end
		end
	end
end
...
function MOD:RefreshInCombatBar()
	...
	if g and g.opt and g.initialized then -- make sure have valid layout
		for i = 1, g.count do
			local v = g.overlays[i]
			local id, buff, duration, expires, _ = v.buffName, nil, 0, 0, nil
			if not g.hidden and (g.opt.ooc or InCombatLockdown()) then -- check if in combat or have out-of-combat enabled
				buff, _, _, _, _, duration, expires = MOD.UnitAuraSpellName("player", id, "HELPFUL")
			end
			if buff then
				local alpha = 1
				if duration and duration > 0 then
...
local function TemporaryEnchantFrame_Update(...)
	...
	for itemIndex = numItems, 1, -1 do --Loop through the items from the back.
		local hasEnchant, enchantExpiration = select(RETURNS_PER_ITEM * (itemIndex - 1) + 1, ...)
		if (hasEnchant) then
	...
	for i = enchantIndex + 1, 3 do
		local tempEnchant = _G["TempEnchant" .. i]
		local tempEnchantDuration = _G["TempEnchant" .. i .. "Duration"]
		if tempEnchant then
			tempEnchant:Hide()
			if tempEnchantDuration then
				tempEnchantDuration:Hide()
			end
		end
	end
	-- Position buff frame
...
local function ReleaseOverlay(bar)
	...
	if b then
		if not InCombatLockdown() then
			DeactivateOverlay(b)
		end -- already deactivated if in combat
		overlayPool[b] = true
		bar.overlay = nil
	end
end
