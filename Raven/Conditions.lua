...
local GetSpecializationInfo = MOD.GetSpecializationInfo
...
local function InRangeSpells(unit, t)
	if t then
		for _, s in pairs(t) do
			if (IsSpellInRange(s, unit) == 1) then
				return true
			end
		end
	end
...
local function UnitRangeCheck(unit)
	if not range_initialized then
		InitializeRangeSpells()
	...
	if UnitCanAttack("player", unit) then -- enemy unit
		if InRangeSpells(unit, range_spells.enemy) then
			return true
		end
	else -- friendly unit
		if UnitIsDeadOrGhost(unit) then
			return InRangeSpells(unit, range_spells.dead)
		end
		if InRangeSpells(unit, range_spells.friend) then
			return true
		end
		if UnitIsUnit(unit, "pet") and InRangeSpells(unit, range_spells.pet) then
			return true
		end
	end
	return false
...
local function CheckSpellReady(spell, unit, rangeCheck, usable)
	...
	if cd and ((cd[4] ~= nil) and (not cd[9] or cd[9] == 0)) then
		return false
	end -- verify is on cooldown and has a valid duration
	if IsOn(rangeCheck) and IsOn(unit) and ((IsSpellInRange(spell, unit) == 1) ~= rangeCheck) then
		return false
	end
	return true
end
...
local function CheckItemReady(item, ready, checkCount, count)
...
function MOD.CheckSpec(spec, specList)
	...
	local currentName = currentSpec and select(2, GetSpecializationInfo(currentSpec))
	if specList then
...
local function CheckTestAND(ttype, t)
	...
	if ttype == "Player Status" then
		...
		if
			IsOn(t.checkComboPoints) and IsOn(t.minComboPoints) and
				(t.checkComboPoints ~= (stat.combo >= t.minComboPoints))
		 then
			return false
		end
	...
	elseif ttype == "Pet Status" then -- pet must exist for these tests to be true
		...
		if
			IsOn(t.checkHealth) and IsOn(t.minHealth) and
				(stat.noPet or (t.checkHealth ~= (stat.petHealth >= t.minHealth)))
		 then
			return false
		end
	...
	elseif ttype == "Target Status" then -- target must exist for these tests to be true
		...
		if
			IsOn(t.classify) and
				(not t.classification or t.classification == "" or stat.noTarget or
					not CheckClassification(stat.targetClassification, t.classification))
		 then
			return false
		end
		...
		if
			IsOn(t.checkHealth) and IsOn(t.minHealth) and
				(stat.noTarget or (t.checkHealth ~= (stat.targetHealth >= t.minHealth)))
		 then
			return false
		end
		if
			IsOn(t.checkPower) and IsOn(t.minPower) and
				(stat.noTarget or (t.checkPower ~= (stat.targetPower >= t.minPower)))
		 then
			return false
		end
		if IsOn(t.inRange) and (stat.noTarget or (t.inRange ~= stat.targetInRange)) then
			return false
		end
	elseif ttype == "Target's Target Status" then -- target's target must exist for these tests to be true
		...
		if
			IsOn(t.classify) and
				(not t.classification or t.classification == "" or stat.noTargetTarget or
					not CheckClassification(stat.targetTargetClassification, t.classification))
		 then
			return false
		end
		if
			IsOn(t.checkMaxHealth) and
				(stat.noTargetTarget or (tonumber(t.maxHealth or 0) or 0) > stat.targetTargetMaxHealth)
		 then
			return false
		end
		if
			IsOn(t.checkHealth) and IsOn(t.minHealth) and
				(stat.noTargetTarget or (t.checkHealth ~= (stat.targetTargetHealth >= t.minHealth)))
		 then
			return false
		end
		if
			IsOn(t.checkPower) and IsOn(t.minPower) and
				(stat.noTargetTarget or (t.checkPower ~= (stat.targetTargetPower >= t.minPower)))
		 then
			return false
		end
		if IsOn(t.inRange) and (stat.noTargetTarget or (t.inRange ~= stat.targetTargetInRange)) then
			return false
		end
	elseif ttype == "Focus Status" then -- focus must exist for these tests to be true
		...
		if
			IsOn(t.classify) and
				(not t.classification or t.classification == "" or stat.noFocus or
					not CheckClassification(stat.focusClassification, t.classification))
		 then
			return false
		end
		if
			IsOn(t.checkHealth) and IsOn(t.minHealth) and
				(stat.noFocus or (t.checkHealth ~= (stat.focusHealth >= t.minHealth)))
		 then
			return false
		end
		if
			IsOn(t.checkPower) and IsOn(t.minPower) and
				(stat.noFocus or (t.checkPower ~= (stat.focusPower >= t.minPower)))
		 then
			return false
		end
		if IsOn(t.inRange) and (stat.noFocus or (t.inRange ~= stat.focusInRange)) then
			return false
		end
	elseif ttype == "Focus's Target Status" then -- focus target must exist for these tests to be true
		...
		if
			IsOn(t.classify) and
				(not t.classification or t.classification == "" or stat.noFocusTarget or
					not CheckClassification(stat.focusTargetClassification, t.classification))
		 then
			return false
		end
		if
			IsOn(t.checkHealth) and IsOn(t.minHealth) and
				(stat.noFocusTarget or (t.checkHealth ~= (stat.focusTargetHealth >= t.minHealth)))
		 then
			return false
		end
		if
			IsOn(t.checkPower) and IsOn(t.minPower) and
				(stat.noFocusTarget or (t.checkPower ~= (stat.focusTargetPower >= t.minPower)))
		 then
			return false
		end
		if IsOn(t.inRange) and (stat.noFocusTarget or (t.inRange ~= stat.focusTargetInRange)) then
			return false
		end
	...
	elseif ttype == "Buff Count" then
		if not CheckAuraValue(t.unit, t.aura, nil, t.count, t.isMine, true, toggle) then
			return false
		end
	elseif ttype == "Buff Type" then
		if IsOn(t.hasBuff) and (MOD:UnitHasBuff("player", t.hasBuff) == toggle) then
			return false
		end
	elseif ttype == "All Debuffs" then
		if HasTable(t.auras) and not CheckAuras(t.unit, t.auras, true, t.isMine, false, toggle) then
			return false
		end
	elseif ttype == "Any Debuffs" then
		if HasTable(t.auras) and not CheckAuras(t.unit, t.auras, false, t.isMine, false, toggle) then
			return false
		end
	elseif ttype == "Debuff Time Left" then
		if not CheckAuraValue(t.unit, t.aura, t.timeLeft, nil, t.isMine, false, toggle) then
			return false
		end
	elseif ttype == "Debuff Count" then
		if not CheckAuraValue(t.unit, t.aura, nil, t.count, t.isMine, false, toggle) then
			return false
		end
	elseif ttype == "Debuff Type" then
		if IsOn(t.hasDebuff) and (MOD:UnitHasDebuff("player", t.hasDebuff) == toggle) then
			return false
		end
	elseif ttype == "All Cooldowns" then
		if HasTable(t.spells) and not CheckAllCooldowns(t.spells, not t.notUsable, t.timeLeft, toggle) then
			return false
		end
	elseif ttype == "Spell Ready" then
		if not CheckSpellReady(t.spell, "target", t.inRange, not t.notUsable) then
			return false
		end
	elseif ttype == "Spell Casting" then
		if not CheckSpellCast(t.spell, t.unit) then
			return false
		end
	elseif ttype == "Item Ready" then
		if not CheckItemReady(t.item, t.toggle, t.checkCount, t.count) then
			return false
		end
	else
		return false
...
local function CheckTestOR(ttype, t)
	...
	if ttype == "Player Status" then
		...
		if IsOn(t.checkTalent) and IsOn(t.talent) and MOD.CheckTalent(t.talent) then
			return true
		end
		if IsOn(t.checkSpec) and IsOn(t.spec) and MOD.CheckSpec(t.spec, t.specList) then
			return true
		end
		if IsOn(t.checkSpell) and IsOn(t.spell) and MOD:CheckSpellStatus(t.spell) then
			return true
		end
		if IsOn(t.hasMainHand) and CheckWeapon(INVSLOT_MAINHAND, t.levelMainHand) then
			return true
		end
		if IsOn(t.hasOffHand) and CheckWeapon(INVSLOT_OFFHAND, t.levelOffHand) then
			return true
		end
	elseif ttype == "Pet Status" then -- pet must exist for these tests to be true
		if IsOn(t.exists) and (t.exists ~= stat.noPet) then
		...
		if IsOn(t.checkTarget) and CheckTarget(t.checkTarget, "pettarget") then
			return true
		end
		if
			IsOn(t.checkHealth) and IsOn(t.minHealth) and
				(not stat.noPet and (t.checkHealth == (stat.petHealth >= t.minHealth)))
		 then
			return true
		end
		if
			IsOn(t.checkPower) and IsOn(t.minPower) and
				(not stat.noPet and (t.checkPower == (stat.petPower >= t.minPower)))
		 then
			return true
		end
		if IsOn(t.checkFamily) and IsOn(t.family) and CheckPetFamily(t.family) then
			return true
		end
		if IsOn(t.checkSpec) and IsOn(t.spec) and CheckPetSpec(t.spec) then
			return true
		end
	elseif ttype == "Target Status" then -- target must exist for these tests to be true
		if IsOn(t.exists) and (t.exists ~= stat.noTarget) then
		...
		if not stat.noTarget then
			if IsOn(t.isPlayer) and (t.isPlayer == stat.targetPlayer) then
				return true
			end
			if IsOn(t.isEnemy) and (t.isEnemy == stat.targetEnemy) then
				return true
			end
			if IsOn(t.isFriend) and (t.isFriend == stat.targetFriend) then
				return true
			end
			if IsOn(t.isNeutral) and (t.isNeutral == stat.targetNeutral) then
				return true
			end
			if IsOn(t.isDead) and (t.isDead == stat.targetDead) then
				return true
			end
			if IsOn(t.isSteal) and (t.isSteal == MOD:UnitHasBuff("target", "Steal")) then
				return true
			end
			if
				IsOn(t.classify) and t.classification and (t.classification ~= "") and
					CheckClassification(stat.targetClassification, t.classification)
			 then
				return true
			end
			if IsOn(t.checkMaxHealth) and ((tonumber(t.maxHealth or 0) or 0) <= stat.targetMaxHealth) then
				return true
			end
			if IsOn(t.checkHealth) and IsOn(t.minHealth) and (t.checkHealth == (stat.targetHealth >= t.minHealth)) then
				return true
			end
			if IsOn(t.checkPower) and IsOn(t.minPower) and (t.checkPower == (stat.targetPower >= t.minPower)) then
				return true
			end
			if IsOn(t.inRange) and (t.inRange == stat.targetInRange) then
				return true
			end
		end
	elseif ttype == "Target's Target Status" then -- target's target must exist for these tests to be true
		...
		if not stat.noTargetTarget then
			if IsOn(t.isPlayer) and (t.isPlayer == stat.targetTargetPlayer) then
				return true
			end
			if IsOn(t.isEnemy) and (t.isEnemy == stat.targetTargetEnemy) then
				return true
			end
			if IsOn(t.isFriend) and (t.isFriend == stat.targetTargetFriend) then
				return true
			end
			if IsOn(t.isDead) and (t.isDead == stat.targetTargetDead) then
				return true
			end
			if IsOn(t.isSteal) and (t.isSteal == MOD:UnitHasBuff("targettarget", "Steal")) then
				return true
			end
			if
				IsOn(t.classify) and t.classification and (t.classification ~= "") and
					CheckClassification(stat.targetTargetClassification, t.classification)
			 then
				return true
			end
			if IsOn(t.checkMaxHealth) and ((tonumber(t.maxHealth or 0) or 0) <= stat.targetTargetMaxHealth) then
				return true
			end
			if IsOn(t.checkHealth) and IsOn(t.minHealth) and (t.checkHealth == (stat.targetTargetHealth >= t.minHealth)) then
				return true
			end
			if IsOn(t.checkPower) and IsOn(t.minPower) and (t.checkPower == (stat.targetTargetPower >= t.minPower)) then
				return true
			end
			if IsOn(t.inRange) and (t.inRange == stat.targetTargetInRange) then
				return true
			end
		end
	elseif ttype == "Focus Status" then -- focus must exist for these tests to be true
		...
		if not stat.noFocus then
			if IsOn(t.isPlayer) and (t.isPlayer == stat.focusPlayer) then
				return true
			end
			if IsOn(t.isEnemy) and (t.isEnemy == stat.focusEnemy) then
				return true
			end
			if IsOn(t.isFriend) and (t.isFriend == stat.focusFriend) then
				return true
			end
			if IsOn(t.isDead) and (t.isDead == stat.focusDead) then
				return true
			end
			if IsOn(t.isSteal) and (t.isSteal == MOD:UnitHasBuff("focus", "Steal")) then
				return true
			end
			if
				IsOn(t.classify) and t.classification and (t.classification ~= "") and
					CheckClassification(stat.focusClassification, t.classification)
			 then
				return true
			end
			if IsOn(t.checkHealth) and IsOn(t.minHealth) and (t.checkHealth == (stat.focusHealth >= t.minHealth)) then
				return true
			end
			if IsOn(t.checkPower) and IsOn(t.minPower) and (t.checkPower == (stat.focusPower >= t.minPower)) then
				return true
			end
			if IsOn(t.inRange) and (t.inRange == stat.focusInRange) then
				return true
			end
		end
	elseif ttype == "Focus's Target Status" then -- focus's target must exist for these tests to be true
		...
		if not stat.noFocusTarget then
			if IsOn(t.isPlayer) and (t.isPlayer == stat.focusTargetPlayer) then
				return true
			end
			if IsOn(t.isEnemy) and (t.isEnemy == stat.focusTargetEnemy) then
				return true
			end
			if IsOn(t.isFriend) and (t.isFriend == stat.focusTargetFriend) then
				return true
			end
			if IsOn(t.isDead) and (t.isDead == stat.focusTargetDead) then
				return true
			end
			if IsOn(t.isSteal) and (t.isSteal == MOD:UnitHasBuff("focustarget", "Steal")) then
				return true
			end
			if
				IsOn(t.classify) and t.classification and (t.classification ~= "") and
					CheckClassification(stat.focusTargetClassification, t.classification)
			 then
				return true
			end
			if IsOn(t.checkHealth) and IsOn(t.minHealth) and (t.checkHealth == (stat.focusTargetHealth >= t.minHealth)) then
				return true
			end
			if IsOn(t.checkPower) and IsOn(t.minPower) and (t.checkPower == (stat.focusTargetPower >= t.minPower)) then
				return true
			end
			if IsOn(t.inRange) and (t.inRange == stat.focusTargetInRange) then
				return true
			end
		end
	elseif ttype == "All Buffs" then
		if HasTable(t.auras) and CheckAuras(t.unit, t.auras, true, t.isMine, true, toggle) then
			return true
		end
	elseif ttype == "Any Buffs" then
		if HasTable(t.auras) and CheckAuras(t.unit, t.auras, false, t.isMine, true, toggle) then
			return true
		end
	elseif ttype == "Buff Time Left" then
		if CheckAuraValue(t.unit, t.aura, t.timeLeft, nil, t.isMine, true, toggle) then
			return true
		end
	elseif ttype == "Buff Count" then
		if CheckAuraValue(t.unit, t.aura, nil, t.count, t.isMine, true, toggle) then
			return true
		end
	elseif ttype == "Buff Type" then
		if IsOn(t.hasBuff) and (MOD:UnitHasBuff("player", t.hasBuff) ~= toggle) then
	...
	elseif ttype == "All Debuffs" then
		if HasTable(t.auras) and CheckAuras(t.unit, t.auras, true, t.isMine, false, toggle) then
			return true
		end
	elseif ttype == "Any Debuffs" then
		if HasTable(t.auras) and CheckAuras(t.unit, t.auras, false, t.isMine, false, toggle) then
			return true
		end
	elseif ttype == "Debuff Time Left" then
		if CheckAuraValue(t.unit, t.aura, t.timeLeft, nil, t.isMine, false, toggle) then
			return true
		end
	elseif ttype == "Debuff Count" then
		if CheckAuraValue(t.unit, t.aura, nil, t.count, t.isMine, false, toggle) then
			return true
		end
	elseif ttype == "Debuff Type" then
		if IsOn(t.hasDebuff) and (MOD:UnitHasDebuff("player", t.hasDebuff) ~= toggle) then
	...
	elseif ttype == "All Cooldowns" then
		if HasTable(t.spells) and CheckAllCooldowns(t.spells, not t.notUsable, t.timeLeft, toggle) then
			return true
		end
	elseif ttype == "Spell Ready" then
		if CheckSpellReady(t.spell, "target", t.inRange, not t.notUsable) then
			return true
		end
	elseif ttype == "Spell Casting" then
		if CheckSpellCast(t.spell, t.unit) then
			return true
		end
	elseif ttype == "Item Ready" then
		if CheckItemReady(t.item, t.toggle, t.checkCount, t.count) then
			return true
		end
	end
	return false
...
local function CheckConditionTests(c)
	if not c.enabled then
		return false
	end -- if not enabled always return false
	if c.tests then
		if c.testLogic then
			local count = 0
			for ttype, test in pairs(c.tests) do
				if test.enable then
					if CheckTestOR(ttype, test) then
						return true
					end
					count = count + 1
				end
			...
			if count > 0 then
				return false
			end -- if any test is enabled and none are true then return false
		else -- must be logical AND
			for ttype, test in pairs(c.tests) do
				if test.enable and not CheckTestAND(ttype, test) then
					return false
				end
			end
		end
...
function MOD:UpdateConditions()
	...
	if instance ~= nil then
		stat.inInstance = (it == "party") or (it == "raid")
		stat.inArena = (it == "arena")
		stat.inBattleground = (it == "pvp")
	else
		stat.inInstance = false
		stat.inArena = false
		stat.inBattleground = false
	end
	...
	if not stat.noPet then
		...
		m = UnitHealthMax("pet")
		if m > 0 then
			stat.petHealth = (100 * UnitHealth("pet") / m)
		else
			stat.petHealth = 0
		end
		m = UnitPowerMax("pet")
		if m > 0 then
			stat.petPower = (100 * UnitPower("pet") / m)
		else
			stat.petPower = 0
		end
	end
	...
	if not stat.noTarget then
		...
		if MOD.LibBossIDs and MOD.CheckLibBossIDs(UnitGUID("target")) then
			classification = "worldboss"
		end
		stat.targetClassification = classification
		m = UnitHealthMax("target")
		if m > 0 then
			stat.targetMaxHealth = m
			stat.targetHealth = (100 * UnitHealth("target") / m)
		else
			stat.targetMaxHealth = 0
			stat.targetHealth = 0
		end
		m = UnitPowerMax("target")
		if m > 0 then
			stat.targetPower = (100 * UnitPower("target") / m)
		else
			stat.targetPower = 0
		end
	...
	if not stat.noTargetTarget then
		...
		if MOD.LibBossIDs and MOD.CheckLibBossIDs(UnitGUID("targettarget")) then
			classification = "worldboss"
		end
		stat.targetTargetClassification = classification
		m = UnitHealthMax("targettarget")
		if m > 0 then
			stat.targetTargetMaxHealth = m
			stat.targetTargetHealth = (100 * UnitHealth("targettarget") / m)
		else
			stat.targetTargetMaxHealth = 0
			stat.targetTargetHealth = 0
		end
		m = UnitPowerMax("targettarget")
		if m > 0 then
			stat.targetTargetPower = (100 * UnitPower("targettarget") / m)
		else
			stat.targetTargetPower = 0
		end
	...
	if not stat.noFocus then
		...
		if MOD.LibBossIDs and MOD.CheckLibBossIDs(UnitGUID("focus")) then
			classification = "worldboss"
		end
		stat.focusClassification = classification
		m = UnitHealthMax("focus")
		if m > 0 then
			stat.focusHealth = (100 * UnitHealth("focus") / m)
		else
			stat.focusHealth = 0
		end
		m = UnitPowerMax("focus")
		if m > 0 then
			stat.focusPower = (100 * UnitPower("focus") / m)
		else
			stat.focusPower = 0
		end
	...
	if not stat.noFocusTarget then
		...
		if MOD.LibBossIDs and MOD.CheckLibBossIDs(UnitGUID("focustarget")) then
			classification = "worldboss"
		end
		stat.focusTargetClassification = classification
		m = UnitHealthMax("focustarget")
		if m > 0 then
			stat.focusTargetHealth = (100 * UnitHealth("focustarget") / m)
		else
			stat.focusTargetHealth = 0
		end
		m = UnitPowerMax("focustarget")
		if m > 0 then
			stat.focusTargetPower = (100 * UnitPower("focustarget") / m)
		else
			stat.focusTargetPower = 0
		end
				...
				if c.result and c.dependencies then -- if starting true then check dependencies
					...
					for dname, result in pairs(c.dependencies) do -- check each dependency in the table
						...
						if c.dependencyType and c.dependencyType[dname] then -- check for at least one non-required dependency to be true
							if IsOn(dep) and (dep.testResult == result) then
								nreqd = true
							end
							ncount = ncount + 1
						else
							if IsOff(dep) or (dep.testResult ~= result) then
								c.result = false
							end -- all required must be true
						end
					end
					if (ncount > 0) and not nreqd then
						c.result = false
					end
				end
				if c.toggleResult and IsOff(c.setResult) then
					c.result = not c.result
				end
			end
		end
	end
...
function MOD:InitializeCondition(name, c, class)
	if class then
		...
		if class ~= MOD.myClass then
			return
		end
	end
	local ct = MOD.DefaultProfile.profile.Conditions
	if ct and name and c then
		if not ct[MOD.myClass] then
			ct[MOD.myClass] = {}
		end
		if c.name == nil then
			c.name = name
		end -- set default values for uninitialized fields
		if c.enabled == nil then
			c.enabled = true
		end
		if c.notify == nil then
			c.notify = true
		end
		if c.tests then
			for ttype, test in pairs(c.tests) do
				...
				if test.spells then
					for k, n in pairs(test.spells) do
						test.spells[k] = GetLocalizedSpellName(n)
					end
				end
				if test.auras then
					for k, n in pairs(test.auras) do
						test.auras[k] = GetLocalizedSpellName(n)
					end
				end
...
function MOD:GetConditionSpell(name)
	...
	if ct then
		local c = ct[name]
		if IsOn(c) and c.associatedSpell and (c.associatedSpell ~= "") then
			return c.associatedSpell
		end
	end
	return nil
end
...
function MOD:GetConditionText(name)
	...
	if not c then
		return description .. " " .. L["(not found)"]
	end
	if not c.enabled then
		etext = L["Disable String"]
	end
		...
		for _, tt in pairs(MOD.testOrder) do -- loop through tests in right order
			...
			if t and t.enable then -- check if a test of this type is defined and enabled
				...
				if tt == "Player Status" then
					if IsOn(t.inCombat) then
						if t.inCombat then
							a = a .. d .. L["In Combat"]
						else
							a = a .. d .. L["Out Of Combat"]
						end
						d = ", "
					end
					if IsOn(t.isResting) then
						if t.isResting then
							a = a .. d .. L["Is Resting"]
						else
							a = a .. d .. L["Not Resting"]
					...
					if IsOn(t.isStealthed) then
						if t.isStealthed then
							a = a .. d .. L["Is Stealthed"]
						else
							a = a .. d .. L["Not Stealthed"]
						end
						d = ", "
					end
					if IsOn(t.isMounted) then
						if t.isMounted then
							a = a .. d .. L["Is Mounted"]
						else
							a = a .. d .. L["Not Mounted"]
					...
					if IsOn(t.inGroup) then
						if t.inGroup then
							a = a .. d .. L["In Group"]
						else
							a = a .. d .. L["Not In Group"]
						end
						d = ", "
					end
					if IsOn(t.inParty) then
						if t.inParty then
							a = a .. d .. L["In Party"]
						else
							a = a .. d .. L["Not In Party"]
						end
						d = ", "
					end
					if IsOn(t.inRaid) then
						if t.inRaid then
							a = a .. d .. L["In Raid"]
						else
							a = a .. d .. L["Not In Raid"]
					...
					if IsOn(t.inInstance) then
						if t.inInstance then
							a = a .. d .. L["In Instance"]
						else
							a = a .. d .. L["Not In Instance"]
						end
						d = ", "
					end
					if IsOn(t.inArena) then
						if t.inArena then
							a = a .. d .. L["In Arena"]
						else
							a = a .. d .. L["Not In Arena"]
						end
						d = ", "
					end
					if IsOn(t.inBattleground) then
						if t.inBattleground then
							a = a .. d .. L["In Battleground"]
						else
							a = a .. d .. L["Not In Battleground"]
						end
						d = ", "
					end
					if IsOn(t.hasPet) then
						if t.hasPet then
							a = a .. d .. L["Has Pet"]
						else
							a = a .. d .. L["No Pet"]
					...
					if IsOn(t.checkLevel) and t.level then
						local x = "<"
						if t.checkLevel then
							x = ">="
						end
						a = a .. d .. L["Level String"](x, t.level)
						d = ", "
					end
					if IsOn(t.checkHealth) and t.minHealth then
						local x = "<"
						if t.checkHealth then
							x = ">="
						end
						a = a .. d .. L["Health String"](x, t.minHealth)
						d = ", "
					end
					if IsOn(t.checkPower) and t.minPower then
						local x = "<"
						if t.checkPower then
							x = ">="
						end
						a = a .. d .. L["Power String"](x, t.minPower)
						d = ", "
					end
					if IsOn(t.checkComboPoints) and t.minComboPoints then
						local x = "<"
						if t.checkComboPoints then
							x = ">="
						end
						a = a .. d .. L["Combo Pts String"](x, t.minComboPoints)
						d = ", "
					end
					if IsOn(t.checkStance) and t.stance then
						a = a .. d .. L["Stance String"](t.stance)
						d = ", "
					end
					if IsOn(t.checkTalent) and t.talent then
						a = a .. d .. L["Talent String"](t.talent)
						d = ", "
					end
					if IsOn(t.checkSpec) and t.spec then
						a = a .. d .. L["Spec String"](t.spec)
						d = ", "
					end
					if IsOn(t.checkSpell) and t.spell then
					...
					if IsOn(t.hasMainHand) and IsOn(t.levelMainHand) then
						a = a .. d .. L["Mainhand String"](t.levelMainHand)
						d = ", "
					end
					if IsOn(t.hasOffHand) and IsOn(t.levelOffHand) then
						a = a .. d .. L["Offhand String"](t.levelOffHand)
						d = ", "
					end
				elseif tt == "Pet Status" then
					if IsOn(t.exists) then
						if t.exists then
							a = a .. d .. L["Exists"]
						else
							a = a .. d .. L["Not Exists"]
					...
					if IsOn(t.checkTarget) then
						if t.checkTarget == "none" then
							a = a .. d .. L["No Target"]
						elseif t.checkTarget == "player" then
							a = a .. d .. L["Player's Target"]
						elseif t.checkTarget == "any" then
							a = a .. d .. L["Any Target"]
						end
						d = ", "
					end
					...
					if IsOn(t.checkFamily) and t.family then
						a = a .. d .. L["Pet Family String"](t.family)
						d = ", "
					end
					if IsOn(t.checkSpec) and t.spec then
						a = a .. d .. L["Pet Spec String"](t.spec)
						d = ", "
					end
				elseif
					tt == "Target Status" or tt == "Target's Target Status" or tt == "Focus Status" or
						tt == "Focus's Target Status"
				 then
					if IsOn(t.exists) then
						if t.exists then
							a = a .. d .. L["Exists"]
					...
					if IsOn(t.isFriend) then
						if t.isFriend then
							a = a .. d .. L["Is Friendly"]
						else
							a = a .. d .. L["Not Friendly"]
						end
						d = ", "
					end
					if IsOn(t.isNeutral) then
						if t.isNeutral then
							a = a .. d .. L["Is Neutral"]
						else
							a = a .. d .. L["Not Neutral"]
					...
					if t.classify and t.classification and (t.classification ~= "") then
						local lc = ClassificationList(t.classification)
						if not lc then
							lc = "Unknown"
						end
						if t.classify then
					...
					if IsOn(t.inRange) then
						if t.inRange then
							...
							a = a .. d .. L["Out Of Range"]
						end
						d = ", "
					end
					if IsOn(t.isSteal) then
						if t.isSteal then
							a = a .. d .. L["Spellsteal"]
						else
							a = a .. d .. L["Not Spellsteal"]
						end
						d = ", "
					end
					if IsOn(t.checkMaxHealth) then
						a = a .. d .. L["Max Health String"](t.maxHealth or 0)
						d = ", "
					end
					if IsOn(t.checkHealth) and t.minHealth then
						local x = "<"
				...
				elseif tt == "All Buffs" or tt == "All Debuffs" then
					if IsOn(t.unit) and t.auras then
						for _, v in pairs(t.auras) do
							a = a .. d .. string.format('"%s"', v)
							d = ", "
						end
						if d == ", " then -- the list of auras contains at least one entry
							if t.toggle == true then
								a = a .. d .. L["Not All Active"]
							else
								a = a .. d .. L["All Active"]
							end
							a = a .. d .. L["On"] .. " " .. unitList[t.unit]
							if t.isMine == true then
								a = a .. d .. L["Cast By Player"]
							else
								if t.isMine == false then
									a = a .. d .. L["Cast By Other"]
								else
									a = a .. d .. L["Cast By Anyone"]
								end
							end
						end
					end
				...
				elseif tt == "Buff Count" or tt == "Debuff Count" then
					if IsOn(t.unit) and t.aura and t.aura ~= "" then
						...
						if IsOff(dcount) then
							dcount = 1
						end -- default count is 1
						if t.toggle == true then
							a = a .. d .. L["Less Than"] .. " " .. dcount
						else
							a = a .. d .. dcount .. " " .. L["Or More"]
						end
						if t.isMine == true then
							a = a .. d .. L["Cast By Player"]
						else
				...
				elseif tt == "All Cooldowns" then
					if IsOn(t.timeLeft) and t.spells then
						...
						if d == ", " then -- the list of spells contains at least one entry
							...
							if t.toggle == true then
								a = a .. d .. L["Less Than"] .. " " .. ds .. L["Seconds"]
							else
								a = a .. d .. ds .. L["Seconds Or More"]
							end
						end
						if t.notUsable == true then
							a = a .. d .. L["Ignore Spell Usable"]
						else
							a = a .. d .. L["Test Spell Usable"]
						end
					end
				elseif tt == "Spell Ready" then
					if t.spell and t.spell ~= "" then
						a = a .. string.format(' "%s"', t.spell)
						d = ", "
						if t.notUsable == true then
							a = a .. d .. L["Ignore Spell Usable"]
						else
						...
						if IsOn(t.inRange) then
							if t.inRange == true then
								a = a .. d .. L["Target In Range"]
							else
								a = a .. d .. L["Target Out Of Range"]
							end
						end
					end
				...
				elseif tt == "Item Ready" then
					if t.item and t.item ~= "" then
						a = a .. string.format(' "%s"', t.item)
						d = ", "
						if t.toggle == false then
							a = a .. d .. L["Item Is Not Ready"]
						elseif t.toggle == true then
							a = a .. d .. L["Item Is Ready"]
						end
						local dcount = t.count
						if IsOff(dcount) then
							dcount = 1
						end -- default value is 1
						if t.checkCount == true then
							a = a .. d .. L["Less Than"] .. " " .. dcount .. " " .. L["Count"]
						elseif t.checkCount == false then
							a = a .. d .. dcount .. " " .. L["Or More"] .. " " .. L["Count"]
						end
					end
				end
	...
	if IsOn(c.dependencies) then -- have a dependencies list
		...
		for _, dep in pairs(list) do
			...
			if c.dependencyType then
				ctype = c.dependencyType[dep]
			end
			a = a .. d .. string.format("[%s] = ", dep)
			if value then
				a = a .. L["True Color String"]
			else
				a = a .. L["False Color String"]
			end
			a = a .. " (" .. (ctype and L["Or"] or L["And"]) .. ")"
			d = ", "
		end
		if a ~= "" then
			depString = depString .. at .. a
		end -- add caption and dependencies
	end
	if c.testLogic then
		logicString = string.format(L["Logic OR String"])
	else
		logicString = string.format(L["Logic AND String"])
	end
	if c.toggleResult then
		resultString = string.format(L["Toggle String"])
	else
		resultString = string.format(L["No Toggle String"])
	end
	if IsOn(c.setResult) then
		logicString = ""
		if c.setResult then
			resultString = L["Set Result True"]
		else
			resultString = L["Set Result False"]
		end
		local a = nil
		if c.toggleShift then
			if c.toggleControl then
		...
		if a then
			resultString = resultString .. L["Modifier Key String"](a)
		end
	end
	local value = MOD:CheckCondition(name)
	if value ~= nil then
		valueString = L["Current Value String"] .. (value and L["True"] or L["False"])
	end
