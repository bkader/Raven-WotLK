...
local MOD = LibStub("AceAddon-3.0"):NewAddon("Raven", "AceConsole-3.0", "AceEvent-3.0", "LibCompat-1.0")
_G.Raven = MOD
...
function MOD:AddTrackers(unit)
	...
	if dstGUID and dstName and not refreshUnits[dstGUID] then
		...
		local name, rank, icon, count, btype, duration, expire, caster, isStealable, _, spellID
...
local function CombatLogTracker(event, ...)
	local timeStamp, e, srcGUID, srcName, sf1, dstGUID, dstName, df1, spellID, spellName, spellSchool, auraType, amount = ...
...
local function AddAura(unit, name, isBuff, spellID, count, btype, duration, caster, steal, icon, expire, tt_type, tt_arg, tt_color, tt_label)
