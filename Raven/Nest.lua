...
local GetPhysicalScreenSize = MOD.GetPhysicalScreenSize

MOD.Nest_SupportedConfigurations = {
	-- table of configurations can be used in dialogs to select appropriate options
	[1] = {
		name = L["Right-to-left bars, label left, icon left"],
		iconOnly = false,
		bars = "r2l",
		label = "left",
		icon = "left"
	...
	[2] = {
		name = L["Left-to-right bars, label left, icon left"],
		iconOnly = false,
		bars = "l2r",
		label = "left",
		icon = "left"
	...
	[3] = {
		name = L["Right-to-left bars, label right, icon left"],
		iconOnly = false,
		bars = "r2l",
		label = "right",
		icon = "left"
	...
	[4] = {
		name = L["Left-to-right bars, label right, icon left"],
		iconOnly = false,
		bars = "l2r",
		label = "right",
		icon = "left"
	...
	[5] = {
		name = L["Right-to-left bars, label left, icon right"],
		iconOnly = false,
		bars = "r2l",
		label = "left",
		icon = "right"
	...
	[6] = {
		name = L["Left-to-right bars, label left, icon right"],
		iconOnly = false,
		bars = "l2r",
		label = "left",
		icon = "right"
	...
	[7] = {
		name = L["Right-to-left bars, label right, icon right"],
		iconOnly = false,
		bars = "r2l",
		label = "right",
		icon = "right"
	...
	[8] = {
		name = L["Left-to-right bars, label right, icon right"],
		iconOnly = false,
		bars = "l2r",
		label = "right",
		icon = "right"
	...
	[9] = {
		name = L["Icons in rows, with right-to-left mini-bars"],
		iconOnly = true,
		bars = "r2l",
		orientation = "horizontal"
	},
	[10] = {
		name = L["Icons in rows, with left-to-right mini-bars"],
		iconOnly = true,
		bars = "l2r",
		orientation = "horizontal"
	},
	[11] = {
		name = L["Icons in columns, right-to-left mini-bars"],
		iconOnly = true,
		bars = "r2l",
		orientation = "vertical"
	},
	[12] = {
		name = L["Icons in columns, left-to-right mini-bars"],
		iconOnly = true,
		bars = "l2r",
		orientation = "vertical"
	},
	[13] = {
		name = L["Icons on horizontal timeline, no mini-bars"],
		iconOnly = true,
		bars = "timeline",
		orientation = "horizontal"
	},
	[14] = {
		name = L["Icons on vertical timeline, no mini-bars"],
		iconOnly = true,
		bars = "timeline",
		orientation = "vertical"
	},
	[15] = {
		name = L["Icons with variable width on horizontal stripe"],
		iconOnly = true,
		bars = "stripe",
		orientation = "horizontal"
	}
...
local anchorDefaults = {
	-- backdrop initialization for bar group anchors
	bgFile = "Interface\\Tooltips\\UI-Tooltip-Background",
	edgeFile = [[Interface\Addons\Raven\Borders\Rounded.tga]],
	tile = true,
	tileSize = ANCHOR_TILESIZE,
	edgeSize = ANCHOR_EDGESIZE,
	insets = {left = 0, right = 0, top = 0, bottom = 0}
}

local iconBackdrop = {
	-- backdrop initialization for icons when using optional border customization
...
local bgTemplate = {
	-- these fields are preserved when a bar group is deleted
	attributes = 0,
	callbacks = 0,
	frame = 0,
	backdrop = 0,
...
local barTemplate = {
	-- these fields are preserved when a bar is deleted
	buttonName = 0,
	attributes = 0,
	callbacks = 0,
	frame = 0,
...
local function GetTukuiFont(font)
	if Raven.db.global.TukuiFont and ChatFrame1 then
		return ChatFrame1:GetFont()
	else
		return font
	end
end
local function PS(x)
	if pixelPerfect and type(x) == "number" then
		return pixelScale * math.floor(x / pixelScale + 0.5)
	else
		return x
	end
end
...
local function PSetSize(frame, w, h)
	if pixelPerfect then
		if w then
			w = pixelScale * math.floor(w / pixelScale + 0.5)
		end
		if h then
			h = pixelScale * math.floor(h / pixelScale + 0.5)
		end
	end
	frame:SetSize(w, h)
end

local function PSetWidth(region, w)
	if pixelPerfect and w then
		w = pixelScale * math.floor(w / pixelScale + 0.5)
	end
	region:SetWidth(w)
end
local function PSetHeight(region, h)
	if pixelPerfect and h then
		h = pixelScale * math.floor(h / pixelScale + 0.5)
	end
	region:SetHeight(h)
end
...
local function PCSetPoint(frame, point, relativeFrame, relativePoint, x, y)
	...
	if pixelPerfect then
		if x then
			x = pixelScale * math.floor(x / pixelScale + 0.5)
		end
		if y then
			y = pixelScale * math.floor(y / pixelScale + 0.5)
		end
	end
...
function MOD.Nest_FlashAlpha(maxAlpha, minAlpha, period)
	...
	if frac >= 0.5 then
		frac = 1 - frac
	end -- now goes from 0 to 0.5 then back to 0
...
local function SetFrameLevel(frame, level)
	...
	repeat
		...
		local a = frame:GetFrameLevel()
		i = i + 1
		if i > 10 then
			print("Raven: warning SetFrameLevel failed")
			return
		end
	until level == a
end
...
local function FaderEffect(bar, toAlpha, fade)
	...
	local fromAlpha = 1 -- for comparison, check target alpha if animation is playing otherwise use current
	...
	if not fade then -- just go straight to the target alpha if fade is disabled
		...
		if delta > 0 then -- use fader animation to get to target alpha
			if isPlaying then
				anim:Stop()
			end -- need to restart the animation with new values if it is playing
			-- alpha:SetFromAlpha(current); -- TODO: FIXME
			alpha:SetChange(-1)
			anim:Play()
...
local function FlashEffect(bar, maxAlpha, minAlpha, period)
	...
	if not anim then
		...
		a:SetDuration(1)
		a:SetOrder(1) -- this is done so that flashing bars can be synchronized
	...
	if anim.maxAlpha ~= maxAlpha or anim.minAlpha ~= minAlpha or anim.flashPeriod ~= period then
		...
		anim:SetScript("OnUpdate", function()
			bar.frame:SetAlpha(FlashAlpha(maxAlpha, minAlpha, period))
		end)
		anim.maxAlpha = maxAlpha
		anim.minAlpha = minAlpha
		anim.flashPeriod = period
	end
...
local function ShowTimeline(bg)
	...
	if back then
		back:Show()
		back.bar:ClearAllPoints()
		back.bar:SetAllPoints(back)
		back.bar:Show()
		if bg.tlTexture then
			back.bar:SetTexture(bg.tlTexture)
		end
		local t = bg.tlColor
		if t then
			back.bar:SetVertexColor(t.r, t.g, t.b, t.a)
		end
		...
		for _, v in pairs(back.labels) do
			if v.hidden then
				v:Hide()
			else
				v:Show()
...
local function sortValues(a, b, f, up)
	...
	if a.gname ~= b.gname then
		return a.gname < b.gname
	end
	if a.sortPlayer then
		if a.isMine ~= b.isMine then
			return a.isMine
		end
	end -- priority #1: optional isMine for cast by player detection
	if math.abs(a[f] - b[f]) >= 0.05 then
		if up then
			return a[f] < b[f]
		else
			return a[f] > b[f]
		end
	end -- priority #2: selected sort function
	if a.sortTime and (math.abs(a.timeLeft - b.timeLeft) >= 0.05) then
		return (a.timeLeft < b.timeLeft)
	end -- priority #3: optional increasing timeLeft
	return a.name < b.name -- priority #4: ascending alphabetic order
end

local function SortTimeDown(a, b)
	return sortValues(a, b, "timeLeft", false)
end
local function SortTimeUp(a, b)
	return sortValues(a, b, "timeLeft", true)
end
local function SortDurationDown(a, b)
	return sortValues(a, b, "duration", false)
end
local function SortDurationUp(a, b)
	return sortValues(a, b, "duration", true)
end
local function SortStartDown(a, b)
	return sortValues(a, b, "start", false)
end
local function SortStartUp(a, b)
	return sortValues(a, b, "start", true)
end
...
function MOD.Nest_RegisterCallbacks(cbs)
	if cbs then
...
local function BarGroup_OnEvent(anchor, callback)
	...
	if bgName then
		bg = barGroups[bgName]
	end -- locate the bar group associated with the anchor
	if bg then
...
local function BarGroup_OnEnter(anchor)
	BarGroup_OnEvent(anchor, "onEnter")
end
local function BarGroup_OnLeave(anchor)
	BarGroup_OnEvent(anchor, "onLeave")
end
...
local function BarGroup_OnMouseDown(anchor, button)
	...
	if (button == "LeftButton") and not IsModifierKeyDown() and bg and not bg.locked then
		bg.startX = PS(bg.frame:GetLeft())
		bg.startY = PS(bg.frame:GetTop())
		bg.moving = true
...
local function BarGroup_OnMouseUp(anchor, button)
	...
	if bg and bg.moving then
		...
		if func then
			local endX = PS(bg.frame:GetLeft())
			local endY = PS(bg.frame:GetTop())
			-- MOD.Debug("moved", bgName, bg.startX, endX, bg.startY, endY)
			PCSetPoint(bg.frame, "TOPLEFT", UIParent, "BOTTOMLEFT", endX, endY)
			if bg.startX ~= endX or bg.startY ~= endY then
				func(anchor, bgName)
			end -- only fires if actually moved
		end
		bg.moving = false
	end
end
...
function MOD.Nest_CreateBarGroup(name)
	if barGroups[name] then
		return nil
	end -- already have one with that name
	...
	if n then
		...
		bg.frame:SetMovable(true)
		bg.frame:SetClampedToScreen(true)
	...
	bg.name = name
	if MSQ then
		bg.MSQ_Group = MSQ:Group("Raven", name)
	end
	bg.update = true
	barGroups[name] = bg
	update = true
...
function MOD.Nest_GetBarGroup(name)
	return barGroups[name]
end

-- Return the table of bar groups
function MOD.Nest_GetBarGroups()
	return barGroups
end
...
function MOD.Nest_DeleteBarGroup(bg)
	...
	for n in pairs(bg.sorter) do
		bg.sorter[n] = nil
	end -- empty the sorting table
	bg.sortFunction = SortAlphaUp
	bg.sortTime = nil
	bg.sortPlayer = nil
	bg.count = 0
	...
	for n in pairs(bg.attributes) do
		bg.attributes[n] = nil
	end
	for n in pairs(bg.callbacks) do
		bg.callbacks[n] = nil
	end
	for n in pairs(bg) do
		if not bgTemplate[n] then
			bg[n] = nil
		end
	end -- remove all settings that don't belong
...
function MOD.Nest_SetBarGroupBarLayout(bg, barWidth, barHeight, iconSize, scale, spacingX, spacingY, iconOffsetX, iconOffsetY, labelOffset, labelInset, labelWrap, labelAlign, labelCenter, labelAdjust, labelAuto, labelWidth, timeOffset, timeInset, timeAlign, timeIcon, iconOffset, iconInset, iconHide, iconAlign, configuration, growDirection, wrap, wrapDirection, snapCenter, fillBars, maxBars, strata)
	bg.barWidth = PS(barWidth)
	bg.barHeight = PS(barHeight)
	bg.iconSize = PS(iconSize)
	bg.scale = scale or 1
	bg.fillBars = fillBars
	bg.maxBars = maxBars
	bg.strata = strata
	bg.spacingX = PS(spacingX or 0)
	bg.spacingY = PS(spacingY or 0)
	bg.iconOffsetX = (iconOffsetX or 0)
	bg.iconOffsetY = PS(iconOffsetY or 0)
	bg.labelOffset = PS(labelOffset or 0)
	bg.labelInset = PS(labelInset or 0)
	bg.labelWrap = labelWrap
	bg.labelCenter = labelCenter
	bg.labelAlign = labelAlign or "MIDDLE"
	bg.labelAdjust = labelAdjust
	bg.labelAuto = labelAuto
	bg.labelWidth = labelWidth
	bg.timeOffset = PS(timeOffset or 0)
	bg.timeInset = PS(timeInset or 0)
	bg.timeAlign = timeAlign or "normal"
	bg.timeIcon = timeIcon
	bg.iconOffset = PS(iconOffset or 0)
	bg.iconInset = PS(iconInset or 0)
	bg.iconHide = iconHide
	bg.iconAlign = iconAlign or "CENTER"
	bg.configuration = configuration or 1
	bg.growDirection = growDirection
	bg.wrap = wrap or 0
	bg.wrapDirection = wrapDirection
	bg.snapCenter = snapCenter
	bg.update = true
end

function MOD.Nest_SetBarGroupSegments(bg, count, override, spacing, hideEmpty, fadeAll, shrinkW, shrinkH, gradient, gradientAll, startColor, endColor, borderColor, advanced, curve, rotate, texture)
	bg.segmentCount = count
	bg.segmentOverride = override
	bg.segmentSpacing = spacing
	bg.segmentAdvanced = advanced
	bg.segmentHideEmpty = hideEmpty
	bg.segmentFadePartial = fadeAll
	bg.segmentShrinkWidth = shrinkW
	bg.segmentShrinkHeight = shrinkH
	bg.segmentGradient = gradient
	bg.segmentGradientAll = gradientAll
	bg.segmentGradientStartColor = startColor
	bg.segmentGradientEndColor = endColor
	bg.segmentBorderColor = borderColor
	bg.segmentCurve = curve
	bg.segmentRotate = rotate
	bg.segmentTexture = texture
	bg.update = true
end

local function TextFlags(outline, thick, mono)
	local t = nil
	if not outline and not thick then
		mono = false
	end -- XXXX workaround for blizzard bugs caused by use of monochrome text flag by itself
	if mono then
		if outline then
			if thick then
				t = "MONOCHROME,OUTLINE,THICKOUTLINE"
			else
				t = "MONOCHROME,OUTLINE"
			end
		else
			if thick then
				t = "MONOCHROME,THICKOUTLINE"
			else
				t = "MONOCHROME"
			end
		end
...
function MOD.Nest_SetBarGroupLabelFont(bg, font, fsize, alpha, color, outline, shadow, thick, mono, special)
	...
	if UseTukui() then
		font = GetTukuiFont(font)
	end
	bg.labelFont = font
	bg.labelFSize = fsize or 9
	bg.labelAlpha = alpha or 1
	bg.labelColor = color
	bg.labelFlags = TextFlags(outline, thick, mono)
	bg.labelShadow = shadow
	bg.labelSpecial = special
	bg.update = true
end
...
function MOD.Nest_SetBarGroupTimeFont(bg, font, fsize, alpha, color, outline, shadow, thick, mono, special)
	...
	bg.timeFont = font
	bg.timeFSize = fsize or 9
	bg.timeAlpha = alpha or 1
	bg.timeColor = color
	bg.timeFlags = TextFlags(outline, thick, mono)
	bg.timeShadow = shadow
	bg.timeSpecial = special
	bg.update = true
end
...
function MOD.Nest_SetBarGroupIconFont(bg, font, fsize, alpha, color, outline, shadow, thick, mono, special)
	if not color then
		color = defaultBackdropColor
	end
	if UseTukui() then
		font = GetTukuiFont(font)
	end
	bg.iconFont = font
	bg.iconFSize = fsize or 9
	bg.iconAlpha = alpha or 1
	bg.iconColor = color
	bg.iconFlags = TextFlags(outline, thick, mono)
	bg.iconShadow = shadow
	bg.iconSpecial = special
	bg.update = true
end
...
function MOD.Nest_SetBarGroupBorder(bg, texture, width, offset, color)
	...
	bg.borderTexture = texture
	bg.borderWidth = PS(width or 0)
	bg.borderOffset = PS(offset or 0)
	bg.borderColor = color
	if bg.borderWidth == 0 then
		bg.borderWidth = PS(1)
	end -- don't let this be zero width
	bg.update = true
end
...
function MOD.Nest_SetBarGroupBackdrop(bg, panel, texture, width, inset, padding, color, fill, offsetX, offsetY, padW, padH)
	...
	bg.backdropPanel = panel
	bg.backdropTexture = texture
	bg.backdropWidth = PS(width or 0)
	bg.backdropInset = PS(inset or 0)
	if bg.backdropWidth == 0 then
		bg.backdropWidth = PS(1)
	end -- don't let this be zero width
	bg.backdropPadding = PS(padding or 0)
	bg.backdropColor = color
	bg.backdropFill = fill
	bg.backdropOffsetX = PS(offsetX or 0)
	bg.backdropOffsetY = PS(offsetY or 0)
	bg.backdropPadW = PS(padW or 0)
	bg.backdropPadH = PS(padH or 0)
	bg.update = true
end
...
function MOD.Nest_SetBarGroupTextures(bg, fgTexture, fgAlpha, bgTexture, bgAlpha, fgNotTimer, fgSaturation, fgBrightness, bgSaturation, bgBrightness)
	bg.fgTexture = fgTexture
	bg.fgAlpha = fgAlpha
	bg.bgTexture = bgTexture
	bg.bgAlpha = bgAlpha
	bg.fgNotTimer = fgNotTimer
	bg.fgSaturation = fgSaturation or 0
	bg.fgBrightness = fgBrightness or 0
	bg.bgSaturation = bgSaturation or 0
	bg.bgBrightness = bgBrightness or 0
	bg.update = true
end
...
function MOD.Nest_SetBarGroupVisibles(bg, icon, cooldown, bar, spark, labelText, timeText)
	bg.showIcon = icon
	bg.showCooldown = cooldown
	bg.showBar = bar
	bg.showSpark = spark
	bg.showLabelText = labelText
	bg.showTimeText = timeText
	bg.update = true
end

-- Set parameters related to timeline configurations
function MOD.Nest_SetBarGroupTimeline(bg, w, h, duration, scale, hide, alt, switch, percent, offset, delta, tex, alpha, color, labels, btex, bw, bo, bc)
	bg.tlWidth = PS(w)
	bg.tlHeight = PS(h)
	bg.tlDuration = duration
	bg.tlScale = scale
	bg.tlHide = hide
	bg.tlAlternate = alt
	bg.tlSwitch = switch
	bg.tlPercent = percent
	bg.tlOffset = offset
	bg.tlDelta = delta
	bg.tlTexture = tex
	bg.tlAlpha = alpha
	bg.tlColor = color
	bg.tlLabels = labels
	bg.tlBorderTexture = btex
	bg.tlBorderWidth = PS(bw or 0)
	bg.tlBorderOffset = PS(bo or 0)
	bg.tlBorderColor = bc
	if bg.tlBorderWidth == 0 then
		bg.tlBorderWidth = PS(1)
	end -- don't let this be zero width
	bg.update = true
end
...
function MOD.Nest_SetBarGroupStripe(bg, fullWidth, w, h, inset, offset, barInset, barOffset, texture, color, btex, bw, bo, bc)
	...
	bg.stInset = inset
	bg.stOffset = offset
	bg.stBarInset = barInset
	bg.stBarOffset = barOffset
	bg.stTexture = texture
	bg.stColor = color
	bg.stBorderTexture = btex
	bg.stBorderWidth = PS(bw or 0)
	bg.stBorderOffset = PS(bo or 0)
	bg.stBorderColor = bc
	bg.stFullWidth = fullWidth
	if bg.stBorderWidth == 0 then
		bg.stBorderWidth = PS(1)
	end -- don't let this be zero width
	bg.update = true
end
...
function MOD.Nest_BarGroupSortFunction(bg, sortMethod, sortDirection, sortTime, sortPlayer)
	if sortMethod == "time" then -- sort by time left on the bar
		if sortDirection then
			bg.sortFunction = SortTimeDown
		else
			bg.sortFunction = SortTimeUp
		end
	elseif sortMethod == "duration" then -- sort by bar duration
		if sortDirection then
			bg.sortFunction = SortDurationDown
		else
			bg.sortFunction = SortDurationUp
		end
	elseif sortMethod == "start" then -- sort by bar start time
		if sortDirection then
			bg.sortFunction = SortStartDown
		else
			bg.sortFunction = SortStartUp
		end
	elseif sortMethod == "class" then -- sort by bar class
		if sortDirection then
			bg.sortFunction = SortClassDown
		else
			bg.sortFunction = SortClassUp
		end
	else -- default is sort alphabetically by bar name
		if sortDirection then
			bg.sortFunction = SortAlphaDown
		else
			bg.sortFunction = SortAlphaUp
		end
	end
	bg.sortTime = sortTime
	bg.sortPlayer = sortPlayer
	bg.update = true
end
...
function MOD.Nest_SetBarGroupTimeFormat(bg, timeFormat, timeSpaces, timeCase)
	bg.timeFormat = timeFormat
	bg.timeSpaces = timeSpaces
	bg.timeCase = timeCase
	bg.update = true
end
...
function MOD.Nest_GetAnchorPoint(bg)
	...
	if left and bottom then
		...
		left = dw / 2
		bottom = dh / 2
	end -- default to center
	local right, top = dw - (left + w), dh - (bottom + h)
	local p = bg.position
	p.left, p.right, p.bottom, p.top, p.width, p.height = left / dw, right / dw, bottom / dh, top / dh, w, h
...
function MOD.Nest_SetAnchorPoint(bg, left, right, bottom, top, scale, width, height)
	if left and bottom and width and height then -- make sure valid settings
		...
		local p = bg.position
		p.left, p.right, p.bottom, p.top, p.width, p.height = left, right, bottom, top, width, height
		local dw, dh = displayWidth, displayHeight
		...
		if right and top and width and height then -- optionally set from other edges if closer to them
			if left > 0.5 then
				xoffset = dw - (right * dw) - width
			end
			if bottom > 0.5 then
				yoffset = dh - (top * dh) - height
			end
		end
		bg.frame:SetScale(scale)
		PSetSize(bg.frame, width, height)
...
function MOD.Nest_SetRelativeAnchorPoint(bg, rTo, rFrame, rPoint, rX, rY, rLB, rEmpty, rRow, rColumn)
	...
	elseif bg.relativeTo and not rTo then -- removing a relative anchor point
		...
		bg.relativeX = rX
		bg.relativeY = rY
		bg.relativeLastBar = rLB
		bg.relativeEmpty = rEmpty
		bg.relativeRow = rRow
		bg.relativeColumn = rColumn
	end
end

-- Set callbacks for a bar group
function MOD.Nest_SetBarGroupCallbacks(bg, onMove, onClick, onEnter, onLeave)
	bg.callbacks.onMove = onMove
	bg.callbacks.onClick = onClick
	bg.callbacks.onEnter = onEnter
	bg.callbacks.onLeave = onLeave
end

-- Set opacity for a bar group, including mouseover override
function MOD.Nest_SetBarGroupAlpha(bg, alpha, mouseAlpha, disableAlpha)
	bg.alpha = alpha or 1
	bg.mouseAlpha = mouseAlpha or 1
	bg.disableAlpha = disableAlpha
end

-- Set a bar group attribute. This is the mechanism to associate application-specific data with bar groups.
function MOD.Nest_SetBarGroupAttribute(bg, name, value)
	bg.attributes[name] = value
end

-- Get a bar group attribute. This is the mechanism to associate application-specific data with bar groups.
function MOD.Nest_GetBarGroupAttribute(bg, name)
...
function MOD.Nest_DeleteBarsWithAttribute(bg, name, value)
	for barName, bar in pairs(bg.bars) do
		if bar.attributes[name] == value then
			MOD.Nest_DeleteBar(bg, bar)
		end
	end
end
...
local function Bar_OnEvent(frame, callback, value)
	...
	if bg then
		...
		if bar then
			if not value then
				value = bar.tooltipAnchor
			end
			local func = bar.callbacks[callback]
			if func then
				func(frame, bgName, name, value)
			end
		end
	end
end
...
local function Bar_OnEnter(frame)
	Bar_OnEvent(frame, "onEnter")
end
local function Bar_OnLeave(frame)
	Bar_OnEvent(frame, "onLeave")
end
local function Bar_OnClick(frame, button)
	Bar_OnEvent(frame, "onClick", button)
end

local function GetButtonName()
	buttonName = buttonName + 1
	return "RavenButton" .. tostring(buttonName)
end -- unique button name

-- Initialize and return a new bar
function MOD.Nest_CreateBar(bg, name)
	if bg.bars[name] then
		return nil
	end -- already have one with that name
	...
	if n then
		...
		bar.backdrop =
			CreateFrame("Frame", bname .. "Backdrop", bar.container, BackdropTemplateMixin and "BackdropTemplate")
		...
		if MSQ then -- if using ButtonFacade, create and initialize a button data table
			...
			for k, v in pairs(MSQ_ButtonData) do
				bar.buttonData[k] = v
			end
		end

		bar.segments = {}
		bar.segmentsAllocated = false
		bar.attributes = {}
		bar.callbacks = {}
	end
...
function MOD.Nest_GetBar(bg, name)
	return bg.bars[name]
end

-- Return the bars table for a bar group
function MOD.Nest_GetBars(bg)
	return bg.bars
end

-- Delete a bar from a bar group, moving it to recycled bar table
function MOD.Nest_DeleteBar(bg, bar)
	if MOD.tooltipBar == bar then
		MOD.tooltipBar = nil
		GameTooltip:Hide()
	end -- disable tooltip update when bar is deleted
	...
	bar.iconBorder:Hide()
	bar.tick:Hide()
	if bar.segmentsAllocated then
		for _, f in pairs(bar.segments) do
			f:ClearAllPoints()
			f:Hide()
		end
	end
	if bar.tukbar then
		bar.tukbar:Hide()
	end -- if skinning for elvui then hide this frame too
	...
	if callbacks.release then
		callbacks.release(bar)
	end
	...
	while i <= bg.count do -- find and remove the corresponding entry in the sorting table
		if bg.sorter[i].name == bar.name then
			if i ~= bg.count then
				bg.sorter[i] = bg.sorter[bg.count]
			end -- copy last one to fill the hole
	...
	for n in pairs(bar) do
		if not barTemplate[n] then
			bar[n] = nil
		end
	end -- remove all settings that don't belong
	for n in pairs(bar.attributes) do
		bar.attributes[n] = nil
	end -- clear bar attributes
	for n in pairs(bar.callbacks) do
		bar.callbacks[n] = nil
	end -- reset bar callbacks
...
function MOD.Nest_StartTimer(bar, timeLeft, duration, maxTime)
	...
	bar.offsetTime = (duration or 0) - timeLeft -- save offset since may be sent multiple times
	bar.timeLeft = timeLeft
	bar.duration = duration or 0
	bar.maxTime = maxTime or duration or 0
	bar.expireDone = nil
	bar.warningDone = nil
	bar.update = true
end
...
function MOD.Nest_IsTimer(bar)
	return bar.timeLeft ~= nil
end

-- Get the time parameters for a bar, including adjusted timeLeft amount
function MOD.Nest_GetTimes(bar)
	return bar.timeLeft, bar.duration, bar.maxTime, bar.startTime, bar.offsetTime
end
...
function MOD.Nest_SetAlpha(bar, alpha)
	bar.alpha = alpha
end

-- Set whether the bar should flash or not
function MOD.Nest_SetFlash(bar, flash)
	bar.flash = flash
end

-- Set whether the bar should have glow effect or not
function MOD.Nest_SetGlow(bar, glow)
	bar.glow = glow
end

-- Set whether the bar should trigger a shine effect
function MOD.Nest_SetShine(bar, shine)
	bar.shine = shine
end

-- Set the label text for a bar
function MOD.Nest_SetLabel(bar, label)
	bar.label = label
end
...
function MOD.Nest_AdjustColor(r, g, b, saturation, brightness)
	if not r or not g or not b then
		return 0.5, 0.5, 0.5
	end -- avoid errors if passed in nil values
	if not saturation then
		saturation = 0
	end
	if not brightness then
		brightness = 0
	end -- set to default values
	if (saturation == 0) and (brightness == 0) then
		return r, g, b
	end
...
local function BarGroup_UpdateAnchor(bg, config)
	...
	if pFrame and GetClickFrame(pFrame) then
		bg.frame:SetParent(pFrame)
	else
	...
	if config.iconOnly then
		lw = rectIcons and bg.barWidth or bg.iconSize
	end -- anchors have same w and h as bars/icons, ignoring icon offsetX
	...
	if config.iconOnly then -- icons can grow in any direction
		if config.orientation == "horizontal" then
			if bg.growDirection then
				align = "BOTTOMRIGHT"
			end -- align rights for going left (growDirection=true), lefts for right (growDirection=false)
		else -- must be "vertical"
			if not bg.growDirection then
				align = "TOPLEFT"
			end -- align bottoms for going up (growDirection=true), tops for down (growDirection=false)
		end
...
local function BarGroup_UpdateBackground(bg, config)
	if config.bars == "stripe" then -- check if drawing a horizontal stripe for icons
		...
		PSetSize(back, w, h)
		PSetSize(back.bar, w, h)
		back:Show()
		back.bar:ClearAllPoints()
		back.bar:SetAllPoints(back)
		back.bar:Show()
		back.anchorPoint = bg.growDirection and "BOTTOM" or "TOP"
		if bg.stTexture then
			back.bar:SetTexture(bg.stTexture)
		end
		local t = bg.stColor
		if t then
			back.bar:SetVertexColor(t.r, t.g, t.b, t.a)
		...
		if bg.stBorderTexture then
			...
			local d = bg.stBorderTable.insets
			d.left = offset
			d.right = offset
			d.top = offset
			d.bottom = offset
			bg.stBorderTable.edgeFile = bg.stBorderTexture
			bg.stBorderTable.edgeSize = bg.stBorderWidth
			back.backdrop:SetBackdrop(bg.stBorderTable)
			local t = bg.stBorderColor
			back.backdrop:SetBackdropBorderColor(t.r, t.g, t.b, t.a)
	...
	elseif config.bars == "timeline" then -- check if drawing the timeine backdrop with panel and numbers
		...
		if config.orientation == "horizontal" then
			w = bg.tlWidth + iconWidth
			h = bg.tlHeight
			edge = "RIGHT"
			justH = "RIGHT"
			justV = "MIDDLE"
			if not bg.growDirection then
				back.anchorPoint = "BOTTOMRIGHT"
				dir = -1
				edge = "LEFT"
				justH = "LEFT"
			end
			...
			w = bg.tlHeight
			h = bg.tlWidth + bg.iconSize
			edge = "TOP"
			justH = "CENTER"
			justV = "TOP"
			if not bg.growDirection then
				back.anchorPoint = "TOPLEFT"
				dir = -1
				edge = "BOTTOM"
				justV = "BOTTOM"
			end
		...
		PSetSize(back, w, h)
		back:SetAlpha(bg.tlAlpha)
		PSetSize(back.bar, w, h)
		if bg.borderTexture then
			local offset = bg.tlBorderOffset
			local d = bg.borderTable.insets
			d.left = offset
			d.right = offset
			d.top = offset
			d.bottom = offset
			bg.borderTable.edgeFile = bg.tlBorderTexture
			bg.borderTable.edgeSize = bg.tlBorderWidth
			back.backdrop:SetBackdrop(bg.borderTable)
			local t = bg.tlBorderColor
			back.backdrop:SetBackdropBorderColor(t.r, t.g, t.b, t.a)
			PSetSize(back.backdrop, w + (2 * offset), h + (2 * offset))
		end
		if type(bg.tlLabels) == "table" then -- table of time values for labels
			...
			for _, v in pairs(bg.tlLabels) do
				...
				if not secs then
					...
					if not start then
						start, m = string.find(v, "[%d%.]+M")
						hidem = true
					end
					if start then
						local nv = string.sub(v, start, m - 1)
						secs = tonumber(nv)
						if secs then
							secs = secs * 60
						end
				...
				if secs and secs <= bg.tlDuration then
					if not back.labelCount then
						back.labelCount = 0
					end
					if i > back.labelCount then
						back.labels = back.labels or {}
						back.labels[i] = back:CreateFontString(nil, "OVERLAY")
						back.labelCount = back.labelCount + 1
					end
					local fs = back.labels[i]
					fs:SetFontObject(ChatFontNormal)
					if ValidFont(bg.labelFont) then
						fs:SetFont(bg.labelFont, bg.labelFSize, bg.labelFlags)
					end
					local t = bg.labelColor
					fs:SetTextColor(t.r, t.g, t.b, bg.labelAlpha)
					fs:SetShadowColor(0, 0, 0, bg.labelShadow and 1 or 0)
					fs:SetText(v)
					fs:SetJustifyH(justH)
					fs:SetJustifyV(justV)
					fs:ClearAllPoints()
			...
			while i <= back.labelCount do
				back.labels[i].hidden = true
				i = i + 1
			end
		end
	end
...
local function Bar_UpdateLayout(bg, bar, config)
	...
	bar.icon:ClearAllPoints()
	bar.iconTexture:ClearAllPoints()
	bar.spark:ClearAllPoints()
	bar.cooldown:ClearAllPoints()
	bf:ClearAllPoints()
	bb:ClearAllPoints()
	...
	bl:SetFontObject(ChatFontNormal)
	bt:SetFontObject(ChatFontNormal)
	bi:SetFontObject(ChatFontNormal)
	bl:SetSize(0, 0)
	bl:SetWordWrap(bg.labelWrap)
	bt:SetJustifyV("MIDDLE")

	if ValidFont(bg.labelFont) then
		bl:SetFont(bg.labelFont, bg.labelFSize, bg.labelFlags)
	end
	if ValidFont(bg.timeFont) then
		bt:SetFont(bg.timeFont, bg.timeFSize, bg.timeFlags)
	end
	if ValidFont(bg.iconFont) then
		bi:SetFont(bg.iconFont, bg.iconFSize, bg.iconFlags)
	end
	...
	if config.iconOnly then -- icon only layouts
		...
		if (bg.barHeight > 0) and (bg.barWidth > 0) and config.bars ~= "timeline" then
			...
			PSetHeight(bf, bg.barHeight)
			PSetHeight(bb, bg.barHeight)
		end
		...
		if bg.labelAdjust then -- bar groups can optionally adjust the label width
			...
			if labelWidth < 30 then
				labelWidth = 30
			end -- enforce minimum width for the label
		end
		...
		if config.label == "right" then
			...
			if lc then
				...
				bl:SetJustifyH("RIGHT")
			end
			if bta == "normal" then
				bt:SetJustifyH(ti and "CENTER" or "LEFT")
			else
				bt:SetJustifyH(bg.timeAlign)
			end
		elseif config.label == "left" then
			...
			if bta == "normal" then
				bt:SetJustifyH(ti and "CENTER" or "RIGHT")
			else
				bt:SetJustifyH(bg.timeAlign)
			end
		end
		...
		if bg.segmentOverride and bat.segmentCount then
			count = bat.segmentCount
		end -- override with bar setting
		if count and (count >= 1) and (count <= 10) then
			if not bar.segmentsAllocated then
				AllocateSegments(bar)
			end
			...
			for i = 1, 10 do
				...
				local fudge = 0
				if (extraPixels < 0) and (i <= -extraPixels) then
					fudge = PS(-1)
				elseif (extraPixels > 0) and (i <= extraPixels) then
					fudge = PS(1)
				end
				...
				if i <= count then
					if texture and textures[texture] then
						...
						sb:SetTexture(textures[texture])
						sb:SetAlpha(bg.bgAlpha)
					else
						f.segmentWidth = sw
						PSetSize(f, sw, bh)
						PSetSize(sf, sw, bh)
	...
	bt:SetText(nil)
	bl:SetText(nil) -- forces update of properties, esp. alignment
	...
	if config.bars ~= "timeline" then
		SetBarFrameLevel(bar, bg.frame:GetFrameLevel() + 5, config.iconOnly)
	end
	if bg.showIcon then
		...
		bi:SetJustifyH(bg.iconAlign)
		bi:SetJustifyV("MIDDLE")
		if MSQ and bg.MSQ_Group and Raven.db.global.ButtonFacadeIcons then -- if using Masque, set custom fields in button data table and add to skinnning group
			...
			if bg.MSQ_Group then
				bg.MSQ_Group:RemoveButton(bar.icon)
			end -- remove skin, if any
			...
			elseif Raven.db.global.PixelIconBorder then -- optional custom pixel perfect border
				...
				bar.icon:SetBackdropColor(t.r, t.g, t.b, t.a)
				bar.iconBorder:SetVertexColor(0, 0, 0, 0)
			else
				bar.icon:SetBackdrop(nil)
				PSetSize(bar.cooldown, iconWidth, bg.iconSize)
				PSetPoint(bar.cooldown, "CENTER", bar.icon, "CENTER")
				IconTextureTrim(
					bar.iconTexture,
					bar.icon,
					(UseTukui() or Raven.db.global.TrimIcon),
					iconWidth,
					bg.iconSize
				)
				bar.iconBorder:SetVertexColor(0, 0, 0, 0)
	...
	PSetSize(bar.frame, w, h)
	PSetSize(bar.container, w, h)
	bar.container:SetAllPoints()
	...
	if bg.showBar and bg.borderTexture and not bat.header and bar.includeBar then
		...
		d.top = offset
		d.bottom = offset
		bg.borderTable.edgeFile = bg.borderTexture
		bg.borderTable.edgeSize = bg.borderWidth
		...
		local t = bg.borderColor
		bar.backdrop:SetBackdropBorderColor(t.r, t.g, t.b, t.a)
		bar.backdrop:Show()
	else
...
function MOD.Nest_FormatTime(t, timeFormat, timeSpaces, timeCase)
	if not timeFormat or (timeFormat > #MOD.Nest_TimeFormatOptions) then
		timeFormat = 24
	end -- default to most compact
	...
	if func then -- check for custom time formatting options
		...
		if t >= 86400 then -- special case for more than one day which applies regardless of selected format
			...
			if (d >= 2) then
				f = string.format("%.0fd", d)
			else
				f = string.format("%.0fd %.0fh", d, h)
			end
		else
			h = math.floor(t / 3600)
...
function MOD.Nest_ValidateCustomTimeFormatFunction()
	...
	userDefinedTimeFormatFunction = nil
	p.userDefinedSample = nil
	p.userDefinedMessage = L["User-defined function not valid"]
	if p.customTimeFormat and s and s ~= "" then -- maybe have a valid user-defined function
		...
		if not func then
			...
			elseif type(uf(1)) ~= "string" then
				...
				p.userDefinedSample =
					uf(8125.8) .. ", " .. uf(343.8) .. ", " .. uf(75.3) .. ", " .. uf(42.7) .. ", " .. uf(3.6)
...
function MOD.Nest_SampleCustomTimeFormatFunction()
	return [[-- sample function that converts the value t in seconds to a custom formatted string and returns the string
...
local function Bar_UpdateSettings(bg, bar, config)
	...
	if not bat.hideLabel then
		labelText = bar.label
	end -- optionally suppress label for a custom bar
	if bar.timeLeft and bar.duration and bar.maxTime and bar.offsetTime then -- only update if key parameters are set
		...
		if (remaining < 0) or bat.ghostTime then
			remaining = 0
		end -- make sure no rounding funnies and make sure ghost bars show 0 time
		if remaining > bar.duration then
			remaining = bar.duration
		end -- and no inaccurate durations!
		bar.timeLeft = remaining -- update saved value
		if remaining < bar.maxTime then
			fill = remaining / bar.maxTime
		end -- calculate fraction of time remaining
		if bg.fillBars then
			fill = 1 - fill
		end -- optionally fill instead of empty bars
	...
	elseif bar.value and bar.maxValue then
		if bar.value < 0 then
			bar.value = 0
		end -- no negative values
		if bar.value < bar.maxValue then
			fill = bar.value / bar.maxValue
		end -- adjust foreground bar width based on values
		if bg.fillBars then
			fill = 1 - fill
		end -- optionally fill instead of empty bars
		if bar.valueText then
			timeText = bar.valueText
		else
			timeText = tostring(bar.value)
		end -- set time text to value or override with value text
		if not bat.hideLabel and bar.valueLabel then
			labelText = bar.valueLabel
		end
		if bar.tickEnable and bar.tickOffset then
			bar.duration = bar.maxValue
			bar.maxTime = bar.duration
		end -- if value has tick then maxValue is duration in seconds
	end

	if bg.showIcon and not isHeader then
		offsetX = bg.iconSize
		if bar.iconPath and not bar.hideIcon then
			bar.icon:Show()
			bar.iconTexture:SetTexture(bar.iconPath)
		else
			bar.icon:Hide()
		end
		...
		if bar.shine then
			ShineEffect(bar, bat.shineColor)
		end -- trigger shine animation
		...
		if MSQ and Raven.db.global.ButtonFacadeIcons then -- icon border coloring
			if Raven.db.global.ButtonFacadeIcons and Raven.db.global.ButtonFacadeBorder and bx and bx.SetVertexColor then
				bx:SetVertexColor(bar.ibr, bar.ibg, bar.ibb, bar.iba)
				showBorder = true
			end
			local nx = MSQ:GetNormal(bar.icon)
			if Raven.db.global.ButtonFacadeNormal and nx and nx.SetVertexColor then
				nx:SetVertexColor(bar.ibr, bar.ibg, bar.ibb, bar.iba)
			end
		else
	...
	if showBorder and bar.iconPath then
		bx:Show()
	else
		bx:Hide()
	end
	if bg.showIcon and not bg.iconHide and not isHeader and bar.iconCount then
		bi:SetText(tostring(bar.iconCount))
		bi:Show()
	else
		bi:Hide()
	end
	if
		bg.showIcon and not isHeader and bg.showCooldown and config.bars ~= "timeline" and bar.timeLeft and
			(bar.timeLeft >= 0)
	 then
		bar.cooldown:SetReverse(bag.clockReverse)
		bar.cooldown:SetCooldown(bar.startTime - bar.offsetTime, bar.duration)
		bar.cooldown:Show()
	else
		bar.cooldown:Hide()
	end

	if bg.showTimeText then
		bt:SetText(timeText)
		bt:Show()
	else
		bt:Hide()
	end
	if (bg.showLabelText or isHeader) and bar.label then
		bl:SetText(labelText)
		bl:Show()
	else
		bl:Hide()
	end
	...
	if bg.showBar and bar.includeBar and (config.bars ~= "timeline") then
		local w, h = bg.width - offsetX, bg.height
		if config.iconOnly then
			w = bg.barWidth
			h = bg.barHeight
		end
		local bf_r, bf_g, bf_b =
			MOD.Nest_AdjustColor(bar.cr, bar.cg, bar.cb, bg.fgSaturation or 0, bg.fgBrightness or 0)
		local bb_r, bb_g, bb_b =
			MOD.Nest_AdjustColor(bar.br, bar.bg, bar.bb, bg.bgSaturation or 0, bg.bgBrightness or 0)
		local count = bg.segmentCount -- create, size and position segments on demand
		if bg.segmentOverride and bat.segmentCount then
			count = bat.segmentCount
		end -- override with bar setting
		if count and (count >= 1) and (count <= 10) then
			...
			for i = 1, count do
				...
				if bg.segmentGradient and (count > 1) then -- override standard foreground color with gradient
					...
					if bg.segmentGradientAll then
						sc = fill
					end -- for colors all together, select based on current value
					local c1, c2 =
						bg.segmentGradientStartColor or defaultGreen,
						bg.segmentGradientEndColor or defaultRed
					bf_r, bf_g, bf_b = MOD.Nest_IntermediateColor(c1.r, c1.g, c1.b, c2.r, c2.g, c2.b, sc)
				end
				sf:SetVertexColor(bf_r, bf_g, bf_b)
				sb:SetVertexColor(bb_r, bb_g, bb_b)
				if i <= (fullSegments + ((px >= (1 / f.segmentWidth)) and 1 or 0)) then -- configure all visible segments including partial
					sf:SetAlpha(bg.fgAlpha)
					PSetSize(sf, f.segmentWidth, f.segmentHeight) -- settings that might be changed for partial segments
					if i > fullSegments then -- partial segment
						if bg.segmentFadePartial then
							sf:SetAlpha(bg.fgAlpha * px)
						end
						if bg.segmentShrinkWidth then
							PSetWidth(sf, f.segmentWidth * px)
						end
						if bg.segmentShrinkHeight then
							PSetHeight(sf, f.segmentHeight * px)
						end
					end
					sf:Show()
					sb:Show()
		...
		elseif (w > 0) and (h > 0) then -- non-zero dimensions to fix the zombie bar bug
			bb:SetVertexColor(bb_r, bb_g, bb_b, 1)
			bb:SetTexture(bg.bgTexture)
			bb:SetAlpha(bg.bgAlpha)
			PSetWidth(bb, w)
			bb:SetTexCoord(0, 1, 0, 1)
			bb:Show()
			if bar.tukbar then
				bar.tukbar:SetAllPoints(bb)
				bar.tukbar:Show()
			end -- elvui backdrop is under the background texture
			hideBar = false

			local fillw = w * fill
			local showfg = bg.fgNotTimer
			if bat.fullReverse then
				showfg = not showfg
			end
			if (fillw > 0) and (showfg or bar.timeLeft) then
				bf:SetVertexColor(bf_r, bf_g, bf_b, 1)
				bf:SetTexture(bg.fgTexture)
				bf:SetAlpha(bg.fgAlpha)
				if fillw > 0 then
					bf:SetWidth(fillw)
				end -- doesn't get pixel perfect treatment
				if bg.showSpark and fill < 1 and fillw > 1 then
					sparky = true
				end
				if config.bars == "r2l" or config.bars == "stripe" then
					bf:SetTexCoord(0, 0, 0, 1, fill, 0, fill, 1)
				else
					bf:SetTexCoord(fill, 0, fill, 1, 0, 0, 0, 1)
				end
				bf:Show()
				if
					bar.tickEnable and bar.tickOffset and bar.tickOffset > 0 and bar.duration and bar.duration > 0 and
						bar.maxTime
				 then -- show tick mark if enabled
					local tickFill = bar.tickOffset / bar.maxTime -- fraction of visible bar that represents the expire time
					if bg.fillBars then
						tickFill = 1 - tickFill
					end -- growDirection if filling instead of emptying bars
					...
					if (offset > 1) and (offset < (w - 1)) then
						bar.tick:SetVertexColor(bar.tr, bar.tg, bar.tb, bar.ta)
						if config.bars == "r2l" then
	...
	if bar.glow then
		GlowEffect(bar, bat.glowColor)
	else
		ReleaseGlowEffect(bar)
	end -- enable or disable glow effect

	local alpha = bar.alpha or 1 -- adjust by bar alpha
	local fade = true
	if bat.header and bag.headerGaps then
		alpha = 0
		fade = false
	end -- header bars can be made to disappear to create gaps
	...
	if not isHeader and (bag.noMouse or (bag.iconMouse and not bg.showIcon)) then -- non-interactive or "only icon" but icon disabled
		bar.icon:EnableMouse(false)
		bar.frame:EnableMouse(false)
		btf:EnableMouse(false)
		if callbacks.deactivate then
			callbacks.deactivate(bar.overlay)
		end
	elseif not isHeader and bag.iconMouse then -- only icon is interactive
		...
		if callbacks.activate then
			callbacks.activate(bar, bar.icon)
		end
		...
		if config.iconOnly and (bg.barWidth > 0) and (bg.barHeight > 0) then -- enable interactivity with potentially separated bars in icon configs
			...
			btf:EnableMouse(false)
			if callbacks.activate then
				callbacks.activate(bar, bar.frame)
			end
		end
	end
	...
	if bat.header and not bag.headerGaps then
		...
		if id == UnitGUID("mouseover") then
			tag = "|cFF73d216@|r"
		end
		if id == UnitGUID("target") then
			tag = tag .. " |cFFedd400target|r"
		end
		if id == UnitGUID("focus") then
			tag = tag .. " |cFFf57900focus|r"
		end
		if tag ~= "" then
			bt:SetText(tag)
			bt:Show()
...
local function Bar_RefreshAnimations(bg, bar, config)
	...
	if bar.timers then -- check special effect timers and force updates so that they happen on time
		for k, t in pairs(bar.timers) do
			if t <= now then
				MOD:ForceUpdate()
				forced = true
				break
	...
	if bg.showBar and bar.includeBar and (config.bars ~= "timeline") then
		...
		elseif (fill > 0) and (showfg or bar.timeLeft) then
			local bf, w, h = bar.fgTexture, bg.width - offsetX, bg.height
			if config.iconOnly then
				w = bg.barWidth
				h = bg.barHeight
			end
...
local function BarGroup_RefreshTimeline(bg, config)
	...
	local maxBars = bg.maxBars
	if not maxBars or (maxBars == 0) then
		maxBars = bg.count
	end
	local back, level, t, lastBar = bg.background, bg.frame:GetFrameLevel() + 5, GetTime(), nil
	local w, h, edge, lastDelta, lastLevel
	if config.orientation == "horizontal" then
		w = bg.tlWidth
		h = bg.tlHeight
		edge = bg.growDirection and "RIGHT" or "LEFT"
	else
		w = bg.tlHeight
		h = bg.tlWidth
		edge = bg.growDirection and "TOP" or "BOTTOM"
	end
	...
	for i = 1, bg.count do
		...
		if i <= maxBars then
			...
			local isOverlap =
				i > 1 and lastBar and math.abs(delta - lastDelta) < (bg.iconSize * (1 - ((bg.tlPercent or 50) / 100)))
			if isOverlap then
				overlapCount = overlapCount + 1
			else
				overlapCount = 0
			end -- number of overlapping icons
			if bg.tlAlternate and isOverlap then
				...
				if switchCount == 1 then
					if (phase % 2) == 1 then
						SetBarFrameLevel(lastBar, clevel, true)
						clevel = lastLevel
					end
				else
			...
			lastBar = bar
			lastLevel = clevel
			local x1 = isVertical and 0 or ((delta - w) * dir)
			local y1 = isVertical and ((delta - h) * dir) or 0
...
local function BarGroup_RefreshStripe(bg)
	...
	for i = 1, bg.count do -- build tables of icons for each alignment, preserving sort order and computing widths
		...
		if bar.icon:IsShown() then
			iw = bar.icon:GetWidth()
		else
			iw = 0
		end
		...
		if bat.minimumWidth and bat.maximumWidth then
			if tw < bat.minimumWidth then
				tw = bat.minimumWidth
			end
			if bat.maximumWidth > 0 and tw > bat.maximumWidth then
				tw = bat.maximumWidth
			end -- ignore max = 0
		end
		...
		hw = tw
		if iw == 0 and bw < tw then
			hw = hw - bw
		end -- compute interactive area width
		if not bag.noMouse and not bag.iconMouse then
			bar.frame:SetHitRectInsets(0, -hw, 0, 0)
		end
		tw = tw + iw -- width of icon plus text plus extra space
		if bat.horizontalAlign == "left" then
			table.insert(alignLeft, bar)
			leftWidth = leftWidth + tw
		elseif bat.horizontalAlign == "center" then
			table.insert(alignCenter, bar)
			centerWidth = centerWidth + tw
		else -- must be default right alignment
			table.insert(alignRight, bar)
			rightWidth = rightWidth + tw
		end
		bar.adjustedWidth = tw -- needed in later passes for each alignment
	end

	if #alignLeft > 1 then
		leftWidth = leftWidth + (sx * (#alignLeft - 1))
	end -- include spacing between icons
	if #alignCenter > 1 then
		centerWidth = centerWidth + (sx * (#alignCenter - 1))
	end
	if #alignRight > 1 then
		rightWidth = rightWidth + (sx * (#alignRight - 1))
	end
	...
	table.wipe(alignLeft)
	table.wipe(alignRight)
	table.wipe(alignCenter) -- clear the temporary tables
end
...
local function BarGroup_SortBars(bg, config)
	...
	for i = 1, bg.count do -- fill data into the sorting table
		...
		if not bar.startTime or not bar.offsetTime then
			s.start = 0
		else
			s.start = bar.startTime - bar.offsetTime
		end
		if not bar.timeLeft or not bar.duration then
			s.timeLeft = unlimited
			s.duration = unlimited
		else
			s.timeLeft = bar.timeLeft
			s.duration = bar.duration
		end
		local id = bar.attributes.group
		if bg.attributes.targetFirst and id and tid and id == tid then
			id = ""
		end -- sorts to front of the list
		s.group = id or ""
		s.gname = bar.attributes.groupName or (bg.growDirection and "zzzzzzzzzzzz" or "")
		s.isMine = bar.attributes.isMine
		s.class = bar.attributes.class or ""
		s.sortPlayer = bg.sortPlayer
		s.sortTime = bg.sortTime
	end
	...
	if not isStripe and count > maxBars then
		count = maxBars
	end
	local ac = count -- actual count before wrap adjustment
	if bg.wrap and not isTimeline then
		wrap = bg.wrap
		if (wrap > 0) and (count > wrap) then
			count = wrap
		end
	end
	...
	if config.iconOnly then -- icons can go any direction from anchor
		if config.orientation == "vertical" then
			...
			if not bg.wrapDirection then
				xoffset = -(bw - 1) * (bg.width + bg.spacingX)
			end
			if bg.snapCenter and bg.locked then
				local z = (dy * (((count - dir) / 2) + cdir))
				y0 = y0 - z
				yoffset = yoffset - z
			end
			if bg.wrapDirection then
				wx = -wx
			end
			if wrap > 0 then -- attachment options differ when wrapping
				bg.lastRow = x0 + (wx * bw)
				bg.lastColumn = y0 + (dy * count)
				bg.lastX = nil
				bg.lastY = nil
			else
				bg.lastX = x0
				bg.lastY = y0 + (dy * count)
				bg.lastRow = nil
				bg.lastColumn = nil
			end
		else -- horizontal
			...
			if not bg.wrapDirection then
				yoffset = -(bh - 1) * (bg.height + bg.spacingY)
			end
			if dir < 0 then
				xoffset = dir * (bw - 1) * (bg.width + bg.spacingX)
				ydir = -ydir
			else
				xoffset = (bw - 1) * (bg.width + bg.spacingX)
				xdir = -1
			...
			if wrap > 0 then -- attachment options differ when wrapping
				bg.lastRow = x0 + (dx * count)
				bg.lastColumn = y0 + (wy * bh)
				bg.lastX = nil
				bg.lastY = nil
			else
				bg.lastX = x0 + (dx * count)
				bg.lastY = y0
				bg.lastRow = nil
				bg.lastColumn = nil
			end
		...
		if bg.showIcon then
			wadjust = bg.iconOffsetX
		end
		if bg.wrapDirection then
			xoffset = wadjust
		else
			xoffset = wadjust - (bw - 1) * (bg.width + bg.spacingX)
		end
		if not bg.locked then
			y0 = y0 + dy
			bh = bh + 1
	...
	if back then
		...
		elseif isStripe then
			...
			local x = bg.stInset
			if bg.stFullWidth then
				x = bg.frame:GetCenter()
				x = (GetScreenWidth() / 2) - x
			end -- keep full width bars centered
	...
	if count > 0 then
		...
		if isTimeline then
			...
			if not bg.locked then
				if config.orientation == "horizontal" then
					w = w + bg.width + bg.spacingX
				else
					h = h + bg.height + bg.spacingY
				end
			end
			if config.orientation == "horizontal" then
				xdir = -xdir
			...
			w = bw * bg.width
			if bw > 1 then
				w = w + ((bw - 1) * bg.spacingX)
			end
			h = bh * bg.height
			if bh > 1 then
				h = h + ((bh - 1) * bg.spacingY)
			end
		end
		local offset = 4
		if (bg.backdropTexture or bg.backdropPanel) then
			offset = bg.backdropPadding
			local x, d = bg.backdropInset, bg.backdropTable.insets
			d.left = x
			d.right = x
			d.top = x
			d.bottom = x
			bg.backdropTable.bgFile = bg.backdropPanel
			bg.backdropTable.edgeFile = bg.backdropTexture
			bg.backdropTable.edgeSize = bg.backdropWidth
			bg.backdrop:SetBackdrop(bg.backdropTable)
			local t = bg.backdropColor
			bg.backdrop:SetBackdropBorderColor(t.r, t.g, t.b, t.a)
			t = bg.backdropFill
			bg.backdrop:SetBackdropColor(t.r, t.g, t.b, t.a)
		else
			bg.backdrop:SetBackdrop(nil)
		end
		PSetSize(bg.backdrop, w + offset - wadjust + bg.backdropPadW, h + offset + bg.backdropPadH)
		xoffset = xoffset + bg.backdropOffsetX
		yoffset = yoffset + bg.backdropOffsetY
	...
	if math.abs(bg.scale - scale) > 0.001 then -- only adjust scale if it has changed by a detectable amount
		if bg.relativeTo then -- if anchored to another bar group then just change the scale
			...
			x0 = bg.frame:GetLeft() * scale
			y0 = bg.frame:GetBottom() * scale -- normalize by scale factor
			bg.frame:SetScale(bg.scale)
			...
			if rbg then
				if rbg.count == 0 then
					...
					while (rbg.count == 0) and rbg.relativeEmpty and rbg.relativeTo do
						...
						if i > 20 then
							break
						end -- safety check, never loop more than 20 deep
					end
				end
				...
				if (rbg.count > 0) or not bg.relativeEmpty then
					...
					if bg.relativeLastBar then -- alternative is to attach to the last bar rendered back in the other bar group
						...
						if rbg.lastX and rbg.lastY then
							offsetX = offsetX + rbg.lastX
							offsetY = offsetY + rbg.lastY
						elseif bg.relativeRow and rbg.lastRow then
							offsetX = offsetX + rbg.lastRow
						elseif bg.relativeColumn and rbg.lastColumn then
							offsetY = offsetY + rbg.lastColumn
						end
					end
				end
...
local function SetBarGroupEffectiveDimensions(bg, config)
	...
	if config.iconOnly then
		w = rectIcons and bg.barWidth or bg.iconSize
		h = bg.iconSize -- icon configs start with icon size and add room for bar and time text, if they are displayed
		h = h + (bg.showBar and (bg.barHeight + math.max(0, bg.iconOffsetY)) or 0)
	else
		if bg.showIcon then
			w = w + bg.iconSize
		end -- bar config start with bar size and add room for icon if it is displayed
	end
	if h < minimumHeight then
		h = minimumHeight
	end -- enforce minimums for dimensions
	if w < minimumWidth then
		w = minimumWidth
	end
	bg.width = PS(w)
	bg.height = PS(h)
	if not bg.scale then
		bg.scale = 1
	end
end
...
function MOD.Nest_UpdatePixelScale(displayReport)
	...
	pixelPerfect =
		(not MOD.db.global.TukuiSkin and MOD.db.global.PixelPerfect) or
		(MOD.db.global.TukuiSkin and MOD.db.global.TukuiScale)
	...
	if MOD.db.global.AdjustUIScale then -- option to adjust "UIscale" for optimized pixel perfect alignment on monitor
		...
		if MOD.db.global.OverrideUIScale and x and (x >= 0.1) and (x <= 1) then
			pscale = x
		end
		...
		if not MOD.db.global.SilentUIScale and displayReport then
			print("Raven: detected display resolution " .. tostring(pixelWidth) .. "x" .. tostring(pixelHeight) .. ", adjusted UI scale from " .. tostring(oscale) .. " to " .. tostring(pscale))
		end
	end
end
...
function MOD.Nest_PixelScale()
	return pixelScale
end

-- Return the actual screen resolution expressed in pixels
function MOD.Nest_ScreenResolution()
...
function MOD.Nest_Update()
	for _, bg in pairs(barGroups) do
		if bg.configuration then -- make sure configuration is valid
			...
			if update or bg.update then
				BarGroup_UpdateAnchor(bg, config)
				BarGroup_UpdateBackground(bg, config)
			end
			for _, bar in pairs(bg.bars) do
		...
		if bg.configuration then -- make sure configuration is valid
			...
			for _, bar in pairs(bg.bars) do
				if not bar.update then
					Bar_RefreshAnimations(bg, bar, config)
				end
			end
			if config.bars == "timeline" then
				BarGroup_RefreshTimeline(bg, config)
