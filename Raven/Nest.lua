...
local function FaderEffect(bar, toAlpha, fade)
	...
	if not anim then
		...
		alpha:SetChange(1); alpha:SetDuration(0.1); alpha:SetOrder(1)
		anim.alpha = alpha
...
local function ShineEffect(bar, color)
	...
	if not a then -- allocate an animation if necessary
		...
		if a then shineEffectPool[a] = nil else
			...
			alpha1:SetChange(0); alpha1:SetDuration(0.05); alpha1:SetOrder(1)
			...
			alpha2:SetChange(-1); alpha2:SetDuration(0.05); alpha2:SetOrder(3)
		end
...
function MOD.Nest_SetBarGroupTimeline(bg, w, h, duration, scale, hide, alt, switch, percent, offset, delta, tex, alpha, color, labels, btex, bw, bo, bc)
	...
	bg.tlSwitch = switch; bg.tlPercent = percent; bg.tlOffset = offset; bg.tlDelta = delta
...
function MOD.Nest_StartTimer(bar, timeLeft, duration, maxTime)
	...
	bar.timeLeft = timeLeft; bar.duration = duration or 0; bar.maxTime = maxTime or duration or 0
