...
function MOD:SetSpellDefaults()
	...
	for id, hex in pairs(MOD.defaultColors) do -- add spell colors with localized names to the profile
	 ...
	 -- sets default color in the shared color table
	end
...
function MOD:SetCooldownDefaults()
	...
	if tabs and tabs > 3 then
		for tab = 4, tabs do -- scan inactive tabs of player spell book for icons
			...
			for i = 1, numSpells do
				...
				local stype, id = SpellBook_GetSpellID(index, book)
				if id then -- make sure valid spell book item
					if stype == 0 then --"SPELL" then -- in this case, id is not the spell id despite what online docs say
	...
	iconCache[L["GCD"]] = "Interface\\Icons\\Trade_Engineering" -- cache special spell with GCD cooldown, must be valid
end
...
function MOD:SetSpellEffectDefaults()
	...
	for _, ec in pairs(MOD.spellEffects) do
		...
		if name and name ~= "" then
			...
			if ec.spell then
				spell = getSpellInfo(ec.spell)
				id = ec.spell
			end -- must be valid
			if ec.talent then
				talent = getSpellInfo(ec.talent)
			end -- must be valid
			local t = {
...
function MOD:GetIcon(name, spellID)
	...
	if id then
		return GetSpellTexture((getSpellInfo(id)))
	...
	if not tex then -- if not found then try to look it up through spell API
		...
		if tex and tex ~= "" then
			...
			if id then
				tex = GetSpellTexture((getSpellInfo(id))) -- then try based on id
				if tex == "" then
					tex = nil
				end
			end
