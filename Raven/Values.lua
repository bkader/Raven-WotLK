...
local mirrorIcons
local iconXP, iconMail, iconCurrency, iconClock, iconLatency, iconFramerate, iconArrow
local iconLevel, iconHealth, iconPower, iconThreat, iconDurability, iconCombat, iconRune, iconRested, iconResting
local iconReputation, iconClassification
...
local function ValueComboPoints(unit, fmt)
	...
	local s = GetFormattedText(fmt, p, 5)
	return true, p, 5, s, nil, iconCombo, nil, nil, c.r, c.g, c.b
end
...
local function ValueReputation(unit, fmt)
	...
	if name and standing and barMin and barMax and barValue then
		...
		reputationTable[2] =
			string.format("|cffffff00%s:|r %s %d/%d (%.0f%%)", name, label, xp, xpmax, (xp / xpmax) * 100)
...
local function ValueCastBar(unit, fmt, spell, options)
	...
	local channel = false
	local name, _, text, icon, startTime, endTime, _, castID, noInterrupt = UnitCastingInfo(checkUnit)
	if not name then
		name, _, text, icon, startTime, endTime, _, noInterrupt = UnitChannelInfo(checkUnit)
		channel = true
	end
	if name then
...
function MOD:InitializeValues()
	...
	mirrorIcons = {
		BREATH = GetSpellTexture(5697),
		DEATH = GetSpellTexture(10848),
		EXHAUSTION = GetSpellTexture(9256),
		FEIGNDEATH = GetSpellTexture(5384)
	}
